...
Implementation status: M14-I.5 completes that required cleanup. The failed `snippet.png` File branch, offscreen Image branch, obsolete Image-only errors/tests, and feasibility-probe runtime/dev UI are absent from active source and packaged output. The validated Text offscreen copy-event path and native Image path remain separate. The post-cleanup real-Chrome smoke test passed, and M14-I is complete at committed/pushed checkpoint `ebe915f`. M14-J is next but not started. Decision 42 and Decision 43 are unchanged.
