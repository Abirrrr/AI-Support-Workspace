...
M14-P.4 may establish only the navigation/workspace shell needed for that entry flow. M15 retains all AI behavior: Merchant Context, request-scoped Context Image attachments, Guidance / Gist, provider-independent model selection, Generate, provider-backed generation, editable Generated Output, Copy, and F3 **Save as Snippet**. The approved M15 layout remains Merchant Context plus attachments, Guidance / Gist, `[Model dropdown] [Generate]`, then Generated Output with `[Save as Snippet] [Copy]` and an editable response. M14-P.4 must not simulate or partially implement those capabilities.
