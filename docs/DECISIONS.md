...
M14-O implements this decision through a focused provider-neutral `GenerateSnippetTags` raw-response port and deterministic application service. No production adapter or automatic save/startup/background invocation is added in M14-O; a future approved orchestration may adapt the existing provider-independent generation capability without changing Snippet domain logic. The existing version-1 sidecar/Backup v7 shape is unchanged. Conditional persistence compares the immutable source snapshot with current authored material atomically, and an in-memory per-Snippet attempt guard prevents older concurrent results from overwriting newer work. The explicit application backfill is maximum 20, concurrency one, cancellable, deterministic by `createdAt` then ID, and not UI-wired. Retrieval fails soft when metadata is absent, malformed, unsupported, stale, or unreadable.
...
M14-P.3 records an intentional product-priority refinement without runtime implementation. At that checkpoint, M14-P.1 delivery performance and M14-P.2 F1/F2 daily-use UX were completed, and the authoritative remaining sequence was **M14-P.3 documentation → M14-P.4 Navigation & Snippet Library UI Completion → M14-O Generated Text Snippet Tags & Retrieval → M14-P final Snippet Completion Gate → M15 AI Workspace**. M14-P remained incomplete until that final post-M14-O gate; M14-P.6/M14-P.6.1 later satisfied the gate and closed M14-P as Principal-approved.
...
## Decision 57: Close Speculative Performance Work Before M14-O

M14-P.1 at `7bc5005` is the authoritative current Snippet-delivery performance audit and optimization checkpoint. Its accepted evidence shows that Text Snippet application planning and serialization are effectively instantaneous relative to human interaction; historical warm Text measurements were below practical timer resolution with p95 around 0.1 ms. Guarded PNG preparation is efficient. Genuine JPEG/WebP decode and PNG re-encode remain slower than PNG but acceptable because Image correctness, quality, and safety take precedence over absolute latency. M14-P.1 materially optimized native request serialization without changing the native protocol or image bytes. One-shot native process startup remains measurable architectural overhead, but current evidence does not justify redesigning the host.

There will be no second broad or speculative performance-optimization milestone before M14-O. Current evidence identifies no meaningful Text application-layer bottleneck, and destabilizing a daily-use delivery path without demonstrated product benefit is not approved. This is an evidence-based closure decision, not abandonment of performance. Text remains the highest performance priority because it is the most frequent daily-use path and must feel effectively instantaneous in normal use; acceptance is based on measured and user-visible behavior, not an arbitrary new millisecond SLA or speculative micro-optimization.

Image optimization remains efficiency-only unless a new Principal architecture decision says otherwise. Speed must not be improved through resizing, cropping, downsampling, reduced resolution or quality, bypassing genuine JPEG/WebP decoding, MIME relabelling, or weakening Decision 42 guards. Some Image latency is acceptable. Quality, correctness, and safety remain higher priorities than absolute Image speed.

The observed one-shot native-process cost is an accepted future architectural opportunity, not current scheduled work. Tens-of-milliseconds startup overhead alone does not authorize a persistent native host, Windows service, background daemon, keepalive process, native protocol expansion, or consolidated long-lived lifecycle. Any such redesign requires separate evidence and Principal architecture approval.

M14-O is retrieval infrastructure, not Snippet-delivery infrastructure. Generated tags and retrieval must never enter the trigger-to-delivery hot path. Trigger expansion must not wait for generated metadata, fingerprinting, retrieval, generated-tag reads, generation, or backfill. M14-O must preserve the accepted M14-P.1 performance boundary.

The final M14-P Snippet Completion Gate must re-run the established Text delivery diagnostic where appropriate, compare results with M14-P.1, exercise a real-Chrome Text trigger-to-delivery workflow in the normal daily-use editor, and confirm that M14-O or later Snippet work caused no meaningful regression. Small synthetic differences do not automatically authorize optimization. If Text still feels effectively immediate, the performance concern closes with no further optimization. If a meaningful user-visible delay exists, the gate first measures and classifies the exact responsible stage—trigger detection, service-worker coordination, clipboard preparation/write, native process contact, cleanup, Automatic Paste, or destination-editor behavior—and creates only a focused follow-up for the proven cause. Text serialization must not be assumed to be the bottleneck.

This decision supersedes Decision 49 only where its historical wording could imply that a separate broad optimization gate is still expected before M15; Decision 49's measurements, classification, and requirement for evidence remain historical authority. M14-P.4/M14-P.4.1/M14-P.4.2 is complete and approved at `52befe0`. M14-P.5 records this decision without implementation. The authoritative sequence is **M14-P.1 complete → M14-P.2 complete → M14-P.3 complete → M14-P.4 complete → M14-P.5 documentation → M14-O Generated Text Snippet Tags & Retrieval → final M14-P Snippet Completion Gate including Text performance regression verification → M15 AI Workspace**. No additional performance milestone is inserted between M14-P.5 and M14-O.

M14-P.6 executes the final automated portion of this decision. The current diagnostic reports warm p95 0 ms and maximum 0.1 ms for plain serialization, rich serialization, and Text planning, consistent with the accepted M14-P.1 effectively-instantaneous class. Static dependency inspection confirms no generated metadata, fingerprint, retrieval, generation, or backfill dependency enters trigger delivery or Library Copy. M14-P.6.1 records the Principal's complete 14-item real-Chrome/manual matrix PASS, including accepted overall Text responsiveness and Image quality/safety. The Text performance final gate therefore passes with no meaningful regression against M14-P.1; no further Snippet performance optimization is currently required, and any future optimization remains evidence-dependent. M14-P is **COMPLETE / PRINCIPAL-APPROVED**. M15 — AI Workspace is next, but its functional implementation has not started.

## Decision 58: M15 Versioned AI Workspace Contracts and Transient Multimodal Boundary

M15-A activates the M15 architecture phase while M15 runtime remains **NOT STARTED**. The normative contract is [AI_WORKSPACE_ARCHITECTURE.md](AI_WORKSPACE_ARCHITECTURE.md). M15 implements behavior inside the M14-P.4 fixed Side Panel order—Merchant Context, Context Images, Guidance / Gist, Model/Generate, Generated Output header actions, editable output—and creates no new primary navigation.

Historical M7 Prompt Builder v1, M8 `GenerationProvider`/Ollama v1, and M9 `OutputWorkflow` v1 remain unchanged. M15 introduces versioned provider-independent replacement contracts for Text-only reference retrieval, prompt assembly v2, model discovery and explicit image capability, typed generation attachments, and generation workflow. Active M15 reference material is Text Snippets only. Knowledge data/repositories/Backup/tests remain compatible but inactive, and Image Snippets remain delivery-only. Authority is Safety/application rules → current Gist → Merchant Context facts → Text Snippet references → defaults. Retrieval uses Context plus Gist text only, preserves M14-O weights and deterministic ordering, excludes usage/recency, and is skipped for image-only requests.

Generate is valid with Context text, one or more valid Context Images, or Gist, provided an opaque discovered model is selected. Images require capability exactly `supported`; `unsupported` and `unknown` block visibly and no image is silently dropped. The only M15 adapter is Ollama, but Workspace/application code does not branch on Ollama. Exact saved-default discovery matches select the model; missing matches leave selection empty without rewriting Settings.

Context Images are original-byte, typed PNG/JPEG/WebP attachments held only in mounted Side Panel memory. Paste into Merchant Context and explicit local file selection are the only acquisition routes. Limits are 4 images, 5 MiB each, 20 MiB combined, 8,192 × 8,192 maximum dimensions, 16,777,216 pixels, and 64 MiB decoded RGBA with overflow-safe fail-closed checks. No resize, crop, downsample, lossy conversion, OCR, captioning, scraping, remote fetch, persistence, Backup, Snippet/Image-Snippet/asset reuse, or new permission is allowed.

Each generation uses one immutable snapshot of inputs, attachments, model, and retrieved references. Only one request may be active. Inputs remain, prior output stays visible and is edit-locked during regeneration, success replaces output, and failure preserves it with a separate error. Copy writes exact currently edited plain text through a project boundary and causes no persistence, usage, delivery, or input side effect.

Save as Snippet uses a cryptographically random, 60-second, one-use, same-extension-origin in-memory `BroadcastChannel` handoff to the existing Options Text authoring workflow. Only the random non-content token may enter the extension Options URL and is removed immediately after validation; output text never enters a URL or durable storage. Only the existing explicit authoring Save persists. Expiry, replay, channel loss, or panel loss fails recoverably with output preserved. If implementation proves `BroadcastChannel` unsuitable, work stops for a new decision rather than substituting persistence or URL content.
