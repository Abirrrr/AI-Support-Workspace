...
- **FUTURE ARCHITECTURAL PERFORMANCE OPPORTUNITY (C):** reducing JPEG/WebP-to-PNG conversion latency and consolidating repeated one-shot native-host contacts remain deferred while hardening foundations are active. Controlled 1440×900 conversions measured about 1.04–1.06 s; content-free host startup measured 67.0 ms median / 76.1 ms p95. Direct PNG preparation is already fast. M14-P must re-measure real-world delivery; meaningful remaining user-visible delay requires a focused pre-M15 optimization gate. M14-M.2 assigns no optimization implementation ID.
...
- **M14-N.2 — Options Activation & Real-Chrome Validation (NEXT):** add only the explicit user-gesture `showDirectoryPicker({mode:'readwrite'})`/reauthorization/status surface and connect it to the M14-N.1 adoption/cadence operations. Repeat final production adapter evidence for restart, permission loss, unavailable location, same/different folder, alarm recovery, verified output, and safe retention. No silent Downloads fallback.
...
M13 is complete at `b76fcb4`/`9a3c7ef`. M14-I is complete at `ebe915f`/`28dcf53`. M14-J is complete and real-browser validated, including Crisp/Intercom no-refresh recovery and repeated-reload duplicate safety. M14-K is closed and complete at `e34cd76`. M14-M.0 remains PASS; M14-M.1 implements the shared Dexie v6/Backup v7 foundation; M14-M.2 is checkpointed at `f4d9ab0`; and M14-M.3/M14-M.3.1 is checkpointed at `2475f8b`. M14-M.4 changes engineering-environment documentation only. M14-N.1 is implemented with automated validation; M14-N.2 is next, while M14-O/P and M15 remain separately gated and unstarted.
