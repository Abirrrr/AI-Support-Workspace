...
- **M14-N.3 — Backup Strategy Simplification & Monthly Reminder (ACTIVE):** cancel the former real-Chrome automatic-backup validation; retain Manual Backup v7; track local successful-export time; show a 30-day advisory; retire production scheduling, selected-folder UI/runtime, retention, and `alarms`; keep historical automatic state dormant and historical files untouched.
