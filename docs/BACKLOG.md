...
- **M14-P.3 — UI/UX Architecture & Roadmap Documentation (ACTIVE / DOCUMENTATION ONLY):** record Decision 56 and synchronize current product ownership for navigation, Settings entry, hidden Knowledge UI with compatibility preservation, Snippet hierarchy/actions/delete confirmation, Image Edit preview repair, and the M14-P.4 → M14-O → final M14-P → M15 sequence. No implementation, test, manifest, permission, schema, or Backup change.
- **M14-P.4 — Navigation & Snippet Library UI Completion (NEXT):** implement toolbar action → global Side Panel, accessible Settings gear → existing Options, UI-only Knowledge hiding, Name/Trigger/separate Type and numeric Usage chips/bounded Details, accessible Delete → Edit → Copy icons, explicit delete confirmation, and retained Image preview/fallback cleanup without changing asset bytes, delivery, replacement/remove/`Ctrl+V`, trigger, schema, Backup, or AI behavior.
...
- Add one compact icon-only Settings gear in the Side Panel title row. It uses `chrome.runtime.openOptionsPage()`, accessible name `Open Settings and Libraries`, visible focus, native keyboard activation, and safe failure feedback. Do not add a full-width **Open Libraries** row or Settings dropdown/menu. Keep Snippet Library, Settings, Import / Export/manual backup, paste behavior, model/provider controls, and future management in Options; hide Knowledge navigation while preserving compatibility data.
...
- Decision 56 assigns this capability to M14-P.4 while preserving Decision 54's Chrome architecture. M14-P.3 documents it only; M15 retains actual AI functionality.
...
M13 is complete at `b76fcb4`/`9a3c7ef`. M14-I is complete at `ebe915f`/`28dcf53`. M14-J is complete and real-browser validated. M14-K is closed at `e34cd76`. M14-M.0 remains PASS; M14-M.1 is the Dexie v6/Backup v7 foundation; M14-M.2 is checkpointed at `f4d9ab0`; M14-M.3/M14-M.3.1 at `2475f8b`; and M14-N.3 is complete at `d410ecb`. M14-P.1 is approved at `7bc5005`; M14-P.2 is approved at `64b5dd8`; M14-P.3 is the active documentation checkpoint, M14-P.4 is next, then M14-O and the final M14-P gate. M15 remains gated and incomplete.
