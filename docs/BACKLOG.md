...
- **M14-M — Usage Statistics Behavior (NOT STARTED):** add the one-use cleanup receipt, best-effort atomic increment, and minimal `<count> uses` UI over the implemented sidecar. Retrieval/view/edit/export never count; failed writes never change delivery.
...
M13 is complete at `b76fcb4`/`9a3c7ef`. M14-I is complete at `ebe915f`/`28dcf53`. M14-J is complete and real-browser validated, including Crisp/Intercom no-refresh recovery and repeated-reload duplicate safety. M14-K is closed and complete at `e34cd76`. M14-M.0 remains PASS, and M14-M.1 has implemented the shared Dexie v6/Backup v7 foundation with automated validation passing. M14-M behavior, M14-N/O/P, and M15 remain separately gated and unstarted.
