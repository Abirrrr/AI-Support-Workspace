...
- Current backup behavior is Manual Backup v7 Export plus local `lastSuccessfulBackupAt` and a 30-elapsed-day advisory recommendation. No success displays `Never`; less than 30 days is normal; exactly 30 days or more is recommended; a successful new export resets the age.
- Success is recorded only after Manual Export completes. Click, preparation, serialization, failure, and cancellation do not record it. The timestamp uses canonical UTC and an injected test clock.
- The reminder is informational and never blocks Snippet use/startup, forces or automatically creates an export, opens a modal loop, requires folder authority, schedules background work, or uses alarms/notifications.
- Manual Export and Backup v7 remain unchanged in scope and compatibility. Historical v7 cadence remains importable but cannot activate automatic behavior or fabricate local backup success/authority.
- Current production exposes no cadence selector, selected-folder status, Choose/Change Folder, Reauthorize, automatic retention/deletion, or unattended filesystem write. Previously created managed files remain untouched. Dormant historical automatic state may remain under Dexie v6 without a destructive migration.
- Generated tags apply only to Text Snippets, remain separate from authored ordered tags, and are non-authoritative. M14-O implements a narrower provider-neutral `GenerateSnippetTags` application port and lifecycle with no production adapter or automatic generation-on-save wiring. Authored material Save invalidates stale metadata transactionally but never blocks, rolls back, guesses/installs a model, or invokes generation.
...
- Implemented deterministic retrieval preserves title/authored-tag/content weights 5/3/1 and adds fingerprint-valid generated tags only at weight 1. Missing, stale, malformed, unsupported, or unreadable metadata contributes zero without blocking ordinary retrieval. Usage, recency, and generated time remain outside score and tie-breaking. Knowledge and Prompt Builder v1 remain unchanged until M15.
...
## M14-P.5 Performance Closure Requirements

- M14-P.1 at `7bc5005` is the accepted current Snippet-delivery performance baseline. Text application planning/serialization is effectively instantaneous relative to normal interaction, guarded PNG preparation is efficient, native request serialization is optimized, and quality-preserving JPEG/WebP decode/re-encode is currently acceptable.
- Text Snippet responsiveness is the highest performance priority because Text is the most frequent daily-use path. Text should feel effectively instantaneous in normal use. This is evaluated through measurement and user-visible behavior, not a newly invented hard millisecond SLA.
- No second broad or speculative Text performance-optimization milestone occurs before M14-O. Performance remains a final completion-gate concern and is not abandoned.
- The final M14-P gate reruns the established Text diagnostic where appropriate, compares with M14-P.1, and checks trigger-to-delivery responsiveness in real Chrome and a normal daily-use editor. Small synthetic changes alone do not require optimization.
- If Text remains effectively immediate, the concern closes without additional optimization. If meaningful user-visible delay exists, the exact responsible stage must be measured and classified before a focused follow-up is authorized; Text serialization is not presumed responsible.
