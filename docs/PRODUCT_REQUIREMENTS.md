...
- Before M14-M.1 commits Dexie v6/Backup v7 or the physical selected-folder state design, M14-M.0 must pass the focused real-Chrome picker/handle/permission/background-write/exact-file/removal/revocation gate using scratch persistence and no `alarms`/`downloads` permission. Failure stops the migration and returns to Principal architecture review.
