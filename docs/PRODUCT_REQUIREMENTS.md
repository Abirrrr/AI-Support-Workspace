...
- M14-L/L.1 assigns implementation-ready architecture to M14-M.0 through M14-P; it implements nothing. Decisions 50–53 are Principal-approved in substance and Decision 54 remains intact. M14-M.1/M14-M.1.1 are checkpointed at `20b509c`, M14-M.2 at `f4d9ab0`, M14-M.3 implements usage behavior, M14-N.1 implements the automatic-backup runtime core, and M14-N.2 implements Options activation/runtime wiring over unchanged Dexie v6/Backup v7. M14-N.3 owns final production real-Chrome validation and M14-N closeout.
- Daily-use F1 requires existing safe-link-mark text to render blue and underlined in the Snippet rich-text editor. This is conventional presentation of current link semantics, not arbitrary color controls, general underline formatting, broader typography, or a persisted Rich Text domain change.
- Daily-use F2 requires Edit to load the selected Snippet, bring its editor into view, and focus the primary appropriate editor field through accessible deterministic behavior. It must preserve current editing/data semantics, require no route redesign, and use no arbitrary timing hack.
- F1 and F2 must be resolved and validated before M14-P closes. M14-P also reviews all Text/Image authoring and delivery behavior, hardening features, import/export, real-world feedback, user-visible performance, regressions, and documentation consistency before M15 begins.
...
- Automatic output uses the same canonical Backup v7 construction as manual export. It never silently falls back to Downloads or prompts from a service worker. M14-N.1 adds `alarms` as the only new Chrome permission; File System Access uses user-granted directory authority and `downloads` is absent.
...
- M14-M.0 is PASS / REAL-CHROME VALIDATED: scratch persistence proved picker, handle, permission, restart, background reuse, exact-file lifecycle/removal, and unavailable-location safety without `alarms`/`downloads`. M14-N.1 implements the production adapter and treats different/uncertain identity fail-safe as a new set; M14-N.2 implements the explicit Options wiring. M14-N.3 must prove the final wired production flow, including positive different-folder behavior, in real Chrome before M14-N closes.
- Backup v7 carries exact preferred cadence `Off | Daily | Weekly` but never directory authority. Profile restore preserves any independently held machine-local folder authorization. Restored Daily/Weekly with no locally authorized folder fabricates none, remains inactive, never prompts or rapidly retries from background, reports location attention, and keeps manual Export available. Explicit folder authorization activates scheduling with that restored cadence; restored Off remains inactive without requiring location attention and retains any existing folder authorization for later reuse.
