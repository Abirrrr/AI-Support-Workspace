...
- **M14-N — Periodic Automatic Backup:** implement `alarms`, exact 24-hour/168-hour scheduling, shared canonical construction, selected-folder output, verified ownership manifest, Daily latest-seven and Weekly latest-four retention, status/failure isolation, and manual Export fallback over the gate-proven directory model. Revalidate the production path in real Chrome. No `downloads` permission or silent Downloads fallback.
...
The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → M14-N → M14-O → M14-P → M15. M14-M.4 is the intervening documentation-only Git environment record and does not alter product sequencing. This decomposition proves the selected-folder premise before permanent schema commitment, records real-world feedback without implementing it, then uses one shared data foundation before feature behavior to avoid unnecessary migrations while keeping usage, automatic backup, and generated tagging independently fail-safe.
