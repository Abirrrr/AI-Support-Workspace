...
## M14-L / M14-L.1 — Snippet Hardening Architecture & Reconciliation (Current / Documentation Only)
...
- **M14-L/M14-L.1 — Snippet Hardening Architecture & Reconciliation (CURRENT / DOCUMENTATION ONLY):** Decisions 50–53 are Principal-approved in substance; Decision 54 remains intact. Define usage, backup, generated-tag/retrieval, coordinated portable evolution, primary navigation, and the pre-migration feasibility gate without implementation.
- **M14-M.0 — Selected-Folder Backup Feasibility Gate:** before any permanent migration, use a scratch/non-production boundary in real Chrome to prove Options-page picker activation, IndexedDB handle cloning/recovery across lifecycle, permission semantics, already-granted background reuse, exact test-file create/close/reopen/verify/remove, unrelated-file preservation, and safe revocation/unavailability with no `alarms`, `downloads`, or arbitrary filesystem access. Failure stops before M14-M.1 and returns to Principal architecture review.
- **M14-M.1 — Coordinated Hardening Persistence & Backup v7 Foundation:** only after M14-M.0 PASS, implement the single Dexie v6 migration, strict Backup v7 DTO/validation/atomic restore, preferred-cadence Settings field, empty sidecars, and the feasibility-proven local automatic-state design. Portable cadence never restores local filesystem authority.
