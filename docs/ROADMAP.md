...
- **M14-P.4/M14-P.4.1/M14-P.4.2 — Navigation & Snippet Library UI Completion (IMPLEMENTED / AUTOMATED PASS / AWAITING REAL-CHROME VALIDATION):** preserves popup-free toolbar-to-Side-Panel, UI-only Knowledge hiding, completed Snippet hierarchy/actions/confirmation, authoritative clipboard-only Copy, and retained Image preview/fallback. M14-P.4.2 adds the approved responsive Workspace presentation and configures the existing Options application for normal-tab display. M15 still owns all provider/model, generation, Prompt Builder, Context Image ingestion, output lifecycle/Copy, and F3 behavior; permissions, native code, trigger behavior, asset bytes, Dexie v6, and Backup v7 remain unchanged.
