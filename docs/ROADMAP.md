...
The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-P.1 focused performance gate → M14-O → F1/F2 daily-use UX cleanup → M14-P final completion gate → M15. M14-M.4 is the intervening documentation-only Git environment record. F3 remains M15 and the canonical trigger remains `;`.
