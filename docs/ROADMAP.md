...
## Milestone 14 — Snippet Authoring and Delivery (Current — M14-K Active)
...
Status: M14-I is complete through `28dcf53`. M14-J is COMPLETE, REAL-BROWSER VALIDATED, and synchronized at `e4e9645`. Crisp Text/Enter/Shift+Enter/Image/no-refresh pass. Intercom Shadow-DOM Text/normal bullet/Image/no-refresh pass. Repeated reload remains duplicate-safe. The accepted Intercom bullet-after-Shift+Enter edge and perceived Image latency are non-blocking follow-ups. M14-K Automatic Paste is active; M14-K.1 has defined the documentation-only architecture and M14-K.2 is the exact next implementation task after Principal approval.
...
- **M14-K.1 — Automatic Paste Architecture + Focus Safety (ACTIVE / ARCHITECTURE DEFINED):** Decision 45 makes Windows automatic paste optional and additive, preserves clipboard-only/manual `Ctrl+V`, selects the existing C# companion instead of a permanent AutoHotkey dependency, and defines layered browser/editor plus native foreground/clipboard/modifier safety, strict protocol v2, no-queue concurrency, and one-shot/no-retry behavior. No runtime implementation is included.
- **M14-K.2 — Windows Automatic Paste Implementation (NEXT AFTER PRINCIPAL APPROVAL):** add the opt-in `snippetPasteMode`, strict Backup v6 with v1-v5 defaulting to clipboard-only, application boundary, one-use activation authorization, global delivery guard, protocol v2, direct Win32 input adapter, typed fallback UX, automated focus/concurrency/fault coverage, and timing instrumentation without changing protocol v1 or clipboard preparation.
- **M14-K.3 — Crisp/Intercom Real-World Validation:** validate automatic and manual Text/Image workflows in both destinations, including tab/window/application/editor focus changes, companion absence, manual fallback, duplicate prevention, and the residual same-window focus race. Release remains blocked if wrong-destination safety is not demonstrated.

The shared Backup v5 transition in M14-G remains frozen/importable and Dexie remains physical version 5. M14-K.2's new Settings preference requires strict Backup v6 for new exports; older imports default to clipboard-only. Manual `Ctrl+V` remains current behavior and permanently supported. The exact next action is Principal review of M14-K.1, followed by M14-K.2 only after approval. No AutoHotkey dependency, `SendInput`, keyboard injection, Settings toggle, protocol-v2 operation, or automatic-paste runtime exists yet. M15 is not started.
...
Decision 45 resolves the previously deferred Optional Windows Automatic Native Paste investigation. The future M14-K.2 implementation attaches after clipboard preparation through the existing companion, retains manual `Ctrl+V`, and forbids a permanent AutoHotkey dependency or arbitrary input surface. It remains separate from Decision 43 protocol v1 and is not implemented by M14-K.1.
