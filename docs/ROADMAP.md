...
## Milestone 14 — Snippet Authoring and Delivery (Current — M14-J.5.1 Documentation Checkpoint)
...
Status: M14-I is complete through `28dcf53`. M14-J.1 through M14-J.5 remain preserved uncommitted work. M14-J.2 is real-Crisp Text PASS, and M14-J.3 Shadow-DOM activation plus ordinary rich Text are real-Intercom PASS. M14-J.4 confirms canonical list serialization and corrects generic inline hard breaks. M14-J.5 proves the first apparent list failure used an invalid two-item fixture; the corrected three-item record passes persistence, serializer, delivery-payload equality, and normal Intercom paste. Crisp and Intercom Image paste pass. Intercom bullet-list triggering after Shift+Enter is a known low-priority compatibility limitation. M14-J.5.1 synchronizes this evidence. M14-J.6 is the exact next task and is not started, so M14-J is incomplete.
...
- **M14-J.1 — Destination Compatibility Validation Baseline (UNCOMMITTED WORK PRESERVED):** controlled fixtures validate input, textarea, generic contenteditable, and a structured nested rich-editor approximation.
...
- **M14-J.5.1 — Destination Validation Documentation Checkpoint (CURRENT):** records Crisp Text/Image PASS, Intercom normal Text/bullet/Image PASS, and Intercom bullet-after-Shift+Enter as a known low-priority compatibility limitation. It preserves M14-J.1–J.5 source/test work unchanged.
- **M14-J.6 — Content Script Lifecycle Recovery & Always-On Availability (NEXT / NOT STARTED):** recover eligible already-open pages after install/update, extension reload/restart, or service-worker recreation without refresh. Recovery must be idempotent and avoid duplicate listeners, subscriptions, or activations. The final host-access architecture remains deferred to this task.

The shared Backup v5 transition in M14-G remains intentional and Dexie remains physical version 5. Manual `Ctrl+V` is accepted. The exact next engineering action is M14-J.6 — Content Script Lifecycle Recovery & Always-On Availability; its final host-access architecture is decided in that task. M14-K follows M14-J and will compare C# native OS paste input with AutoHotkey as feasibility/reference only; no AutoHotkey dependency, `SendInput`, or automatic paste exists. M15 is not started.
