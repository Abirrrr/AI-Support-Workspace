...
- **M14-N.3 — Real-Chrome Validation & Automatic Backup Closeout (NEXT):** revalidate restart, permission persistence/revocation, deleted/unavailable locations, positive same/different-folder behavior, missed scheduling, actual verified output, and real 7/4 retention through the final production path. Different-folder positive evidence and M14-N closeout remain pending this gate.
...
- **M14-P — Snippet Hardening Validation & Closeout:** complete automated migration/import/restore/delivery/failure/ranking/scheduler/retention/privacy coverage plus real-Chrome selected-folder/restart/revocation/missed-schedule validation, F1/F2 daily-use UX corrections, and a Principal review of Text/Image authoring, activation, clipboard-only/automatic delivery, destination compatibility, lifecycle recovery, usage, automatic backup, generated tags/retrieval, import/export, real-world feedback, user-visible performance, regressions, and documentation consistency. Re-measure delivery performance; if meaningful user-visible delay remains, require a focused optimization gate before closeout. M15 begins only after M14-P completes.

The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → M14-N.1 → M14-N.2 → M14-N.3 → M14-O → M14-P → M15. M14-M.4 is the intervening documentation-only Git environment record and does not alter product sequencing. This decomposition proves the selected-folder premise before permanent schema commitment, records real-world feedback without implementing it, then uses one shared data foundation before feature behavior to avoid unnecessary migrations while keeping usage, automatic backup, and generated tagging independently fail-safe.
