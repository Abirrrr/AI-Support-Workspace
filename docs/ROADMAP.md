...
- **M14-M.1 — Coordinated Hardening Persistence & Backup v7 Foundation (NEXT PLANNED GATE / NOT AUTHORIZED):** after separate Principal authorization, implement the single Dexie v6 migration, strict Backup v7 DTO/validation/atomic restore, preferred-cadence Settings field, empty sidecars, and the feasibility-proven local automatic-state design. Portable cadence never restores local filesystem authority.
