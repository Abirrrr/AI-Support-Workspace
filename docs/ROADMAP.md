...
- **M14-P.3 — UI/UX Architecture & Roadmap Documentation (ACTIVE / DOCUMENTATION ONLY):** Decision 56 makes the visible daily-use experience authoritative before generated retrieval. It documents toolbar-to-Side-Panel navigation, the Settings gear, Knowledge UI hiding with compatibility preservation, Snippet hierarchy/actions/delete confirmation, retained Image preview repair, and strict M14-P.4/M14-O/M14-P/M15 ownership without runtime, tests, manifest, permission, schema, or Backup changes.
- **M14-P.4 — Navigation & Snippet Library UI Completion (NEXT IMPLEMENTATION):** implement the popup-free toolbar-to-global-Side-Panel shell and accessible Settings gear; hide Knowledge only from active UI/navigation; present Name, Trigger, separate Text/Image and numeric Usage chips, bounded Details, and accessible Delete → Edit → Copy icons; require delete confirmation; and repair/bound the retained Image Edit preview with intentional failure state while preserving replacement/remove/`Ctrl+V`, stored bytes, image quality, and delivery.
...
- **M14-P — Final Snippet Completion Gate:** after M14-P.4 and M14-O, confirm daily-use UX, Library/navigation, Image editing, accepted performance, generated retrieval, complete regression evidence, synchronized documentation, and appropriate real-world/manual validation. M15 begins only after this gate completes.

The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-P.1 performance → M14-P.2 F1/F2 → M14-P.3 documentation → M14-P.4 visible navigation/Library completion → M14-O generated retrieval → M14-P final completion gate → M15 AI Workspace. M14-M.4 is the intervening documentation-only Git environment record. F3 remains M15 and canonical activation remains `;trigger + Space`.

## Milestone 15 — AI Drafting Workflow Refinement and Multimodal Context

After the final M14-P gate, M15 consumes the M14-P.4 Side Panel shell but owns the actual Decision 46/47 AI workflow: compact auto-growing Merchant Context and Guidance / Gist inputs; the exact empty-state/grounding behavior; Text-Snippet-only active reference retrieval; a provider-independent Model dropdown/application boundary; Generate and provider execution; preserved editable Generated Output; **Save as Snippet** and **Copy** in the output header; and reduced primary-workflow copy. Save as Snippet navigates to Text Snippet authoring with generated response content prefilled, permits user edits to title/trigger/tags/content, and persists only after explicit user Save—generation never auto-creates a Snippet. M15 also adds one or more removable request-scoped Context Images with transient, local-first behavior by default, a provider-independent attachment boundary, capability checks, and explicit unsupported-image feedback. Knowledge compatibility remains non-destructively preserved. Detailed application, provider, persistence/lifetime, migration, and UI architecture remains deferred to future M15 tasks.
