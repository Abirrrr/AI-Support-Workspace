...
Status: M14-G/G.1/G.2/G.2.1 is committed at `672185e`. M14-I through M14-I.5 is uncommitted. Text clipboard/native-paste, Settings companion readiness, and Windows native Image delivery are real-Chrome validated. M14-I.5 removes the failed browser Image transports and A1/A2/B feasibility runtime probes while retaining historical evidence. M14-I is at final review/checkpoint; M14-F remains cancelled and M14-H remains absorbed.
...
- **M14-I.1.3 — Offscreen Text Clipboard Compatibility Correction (implemented, runtime validated, uncommitted):** Text writes exact planned plain + safe HTML through one temporary offscreen copy handler and requires handler plus command success. Real Chrome validates clipboard preparation, cleanup, notice, formatting, and lists.
- **M14-I.1.4 — Image Clipboard Compatibility Correction (implemented, runtime failed, uncommitted):** replaced the proven-failing normal Image Async Clipboard write with one temporary copy event that added a genuine `image/png` File after unchanged Decision 42 validation and conversion. Real Chrome pasted `snippet.png`, not an inline/native image, so the final transport does not satisfy Image Snippet semantics.
- **M14-I.1.5.2 — Native Image Clipboard Feasibility Conclusion (completed, uncommitted):** records A1 `TEXT` as `REAL-CHROME FAILED`, focused extension-page B `VISIBLE IMAGE` as `REAL-CHROME PASS`, and A2/F9 as not run/no longer required. Genuine image writing is proven from a focused extension document, but no transparent normal-trigger route is proven. M14-I.1.4 is `REVERT RECOMMENDED` and not yet reverted.
- **M14-I.2 — Windows Native Clipboard Companion Architecture (DEFINED, uncommitted):** Decision 43 selects a Windows-only optional, exact-origin Native Messaging adapter; self-contained C#/.NET 10 LTS; one-shot `sendNativeMessage`; strict bounded PNG-only protocol; registered PNG plus CF_DIBV5; WIC; companion-owned HWND; bounded contention retry; safe errors/logs; per-user installation; and cleanup plans. No implementation, permission, host, executable, installer, registry change, auto-paste, cross-platform helper, M14-J, or Context Image work occurred.
- **M14-I.3 — Windows Native Clipboard Companion Foundation (IMPLEMENTED, uncommitted):** adds the standalone .NET 10 `win-x64` native project, frozen v1 host, WIC/DIBV5/registered-PNG clipboard foundation, fail-closed caller configuration, HWND/mutex/ownership/retry machinery, language-neutral fixtures, and automated Windows tests. It adds no Chrome integration, permission, host manifest, registry entry, installer, automatic paste, cleanup, or M14-J work.
- **M14-I.4/M14-I.4.1 — Chrome Native Messaging Integration and Capability Correction (IMPLEMENTED, REAL-CHROME VALIDATED, uncommitted):** stable development identity, optional permission and truthful Settings status, callback-aligned service-worker-only native transport, strict TypeScript protocol conformance, development host manifest/publish/HKCU registration, and reversible tooling are implemented. Settings `Ready` and complete native Image delivery passed in real Chrome. Production packaging and automatic paste remain absent.
- **M14-I.5 — Native Image Delivery Cleanup and M14-I Finalization (IMPLEMENTED, CLEANUP COMPLETE, uncommitted):** removes the failed offscreen Async/File Image transports, obsolete Image-only errors/messages/tests, and all feasibility-probe runtime/UI/tests. Preserves the validated Text offscreen copy-event transport, Decision 42, Decision 43, native integration, metadata-only catalogs, and historical evidence. M14-I is pending Principal final review, one post-cleanup smoke test, and checkpoint.
- **M14-J — Rich Text Delivery and Destination Capability Validation:** validate the M14-I native-paste implementation in Crisp, Intercom, representative rich editors, and representative plain editors; record text/HTML formatting fidelity, Image paste behavior, graceful representation fallback, and only evidence-backed compatibility corrections. It does not introduce the base clipboard architecture or normal Rich-image delivery.

The shared Backup v5 transition in M14-G remains intentional and Dexie remains physical version 5. Manual `Ctrl+V` is accepted. The exact next action is Principal final M14-I source/status review, one post-cleanup real-Chrome smoke test, and an authorized Git checkpoint. Optional automatic native paste remains a separate later investigation. M14-J remains blocked until checkpoint.
