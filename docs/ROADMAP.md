...
- **M14-M — Snippet Usage Statistics:** after the shared foundation, implement one-use delivery receipts, non-blocking Text/Image usage writes, delete/restore integrity, and minimal `<count> uses` Library display.
...
The exact order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M → M14-N → M14-O → M14-P → M15. This decomposition proves the selected-folder premise before permanent schema commitment, records real-world feedback without implementing it, then uses one shared data foundation before feature behavior to avoid unnecessary migrations while keeping usage, automatic backup, and generated tagging independently fail-safe.
