...
- **M14-N.3 — Backup Strategy Simplification & Monthly Reminder (IMPLEMENTED / AUTOMATED PASS / AWAITING PRINCIPAL REVIEW):** cancels the former real-Chrome automatic-backup validation, retires production scheduler/folder/retention behavior, removes `alarms`, preserves Manual Backup v7 and historical import compatibility, records local successful-export time, and derives an advisory recommendation at 30 elapsed days. M14-N.1/N.2 remain historical checkpoints.
...
The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-O → real-world Snippet UX/performance cleanup including F1/F2 and remeasurement → M14-P → M15. M14-M.4 is the intervening documentation-only Git environment record. F3 remains M15 and the canonical trigger remains `;`.
