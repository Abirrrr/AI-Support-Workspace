...
The exact order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3 → M14-N → M14-O → M14-P → M15. This decomposition proves the selected-folder premise before permanent schema commitment, records real-world feedback without implementing it, then uses one shared data foundation before feature behavior to avoid unnecessary migrations while keeping usage, automatic backup, and generated tagging independently fail-safe.
