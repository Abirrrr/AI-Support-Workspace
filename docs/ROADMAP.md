...
## Milestone 14 — Snippet Authoring and Delivery (Completed at `e34cd76`)
...
Status: COMPLETE / CLOSED. M14-I is complete through `28dcf53`. M14-J is complete and real-browser validated. M14-K.1 defines Decision 45, M14-K.2 implements it, M14-K.3 is Principal-approved with authoritative real-browser PASS evidence, and M14-K.4 locks Decisions 46–49. The approved M14-K implementation checkpoint is `e34cd76` (`feat: add automatic snippet paste delivery`).
...
- **M14-K.4 — Product Decision Lockdown & AI Handoff (COMPLETE IN `e34cd76` / DOCUMENTATION ONLY):** Decisions 46–49 make Text Snippets the future active AI reference library, retire Knowledge from the future active workflow without deleting compatibility data/code, define Guidance / Gist and compact Workspace behavior, approve request-scoped Context Images and Snippet hardening, and defer class-C optimization/production packaging. It added no runtime, schema, permission, or dependency change.
- **M14-K.5 — Milestone Closeout & Checkpoint Recording (DOCUMENTATION ONLY):** records the approved `e34cd76` implementation checkpoint and closes M14-K without runtime, test, schema, permission, dependency, optimization, Snippet-hardening, or AI implementation changes.

Backup v5 remains frozen/importable and Dexie remains physical version 5. New exports use strict Backup v6; v1-v5 imports default the paste mode to clipboard-only. Manual `Ctrl+V` remains permanently supported, automatic paste remains Windows-only, and AutoHotkey is not required. M14-K is closed at `e34cd76`. Production companion installer/registration, signing, updater, and version migration remain separately approved future distribution work.

## M14-L / M14-L.1 — Snippet Hardening Architecture & Reconciliation (Checkpoint Preserved)

M14-L defines Decisions 50–54 without implementation. It preserves M14-K closure, current Dexie v5/Backup v6/runtime behavior, Knowledge compatibility, and established M15/M16 meanings while assigning the previously unassigned hardening gate to the next available M14 task identifiers. Decision 56 later preserves the locked navigation architecture while moving its shell implementation from M15 to M14-P.4.

Implementation sequence:

- **M14-L/M14-L.1 — Snippet Hardening Architecture & Reconciliation (COMPLETE AT `5450cff` / DOCUMENTATION ONLY):** Decisions 50–53 are Principal-approved in substance; Decision 54 remains intact.
- **M14-M.0 — Selected-Folder Backup Feasibility Gate (PASS / REAL-CHROME VALIDATED):** Principal evidence proves picker activation, scratch handle persistence, reload/restart recovery, permission query, independent service-worker reuse, exact owned-file lifecycle/removal, same-folder identity, unrelated-file isolation, and unavailable-location safety without production permission expansion. Different-folder distinction remains a non-blocking M14-N proof item before retention reliance.
- **M14-M.1/M14-M.1.1 — Coordinated Hardening Persistence & Backup v7 Foundation (COMPLETE / CHECKPOINTED AND SYNCHRONIZED AT `20b509c`):** implements the single additive Dexie v6 migration, strict Backup v7 DTO/validation/atomic portable restore, exact `off | daily | weekly` cadence with `weekly` default/migration, focused sidecar repositories, atomic deletion/invalidation, and the smallest feasibility-proven local automatic-state boundary. Restore preserves separately owned local authorization, and portable cadence never contains or creates filesystem authority. No usage delivery/UI, scheduler/output/retention, generated-tag AI/retrieval, permission, or M15 behavior is included.
- **M14-M.2 — Real-World Snippet Feedback & Completion-Gate Recording (DOCUMENTATION ONLY):** records F1 conventional blue/underlined presentation for the existing safe-link mark and F2 deterministic Edit-to-editor view/focus as required pre-M14-P daily-use UX corrections. It assigns F3 user-controlled **Save as Snippet** from future Generated Output to M15 and implements none of the feedback.
- **M14-M.3/M14-M.3.1 — Snippet Usage Statistics Behavior and Display Simplification (COMPLETE / CHECKPOINTED AT `2475f8b`):** adds 30-second transient service-worker receipts after clipboard success, acknowledgement only after exact cleanup, exact request/Snippet/kind/sender/catalog binding, atomic saturating best-effort sidecar writes, Text/Image plus clipboard-only/automatic parity, worker-loss undercount safety, and a numeric-only Library count with an accessible label. It changes no authored Snippet, retrieval, Backup v7, permission, or dependency.
- **M14-N.1 — Automatic Backup Runtime Core (IMPLEMENTED / AUTOMATED PASS):** adds only `alarms`; exact anchor-based 24-hour/168-hour one-shot scheduling; startup/missed-run reconciliation; shared canonical Backup v7 construction; production selected-directory query/identity/write/read/delete boundaries; verified bounded ownership manifest; Daily latest-seven and Weekly latest-four retention; and in-memory plus persisted 30-minute lease safety. No picker/prompt, directory scan, `downloads`, repeating alarm, polling, or keepalive is present.
- **M14-N.2 — Automatic Backup Options Activation & Runtime Wiring (IMPLEMENTED / AUTOMATED PASS):** exposes Off/Daily/Weekly, explicit Choose/Change Folder and Reauthorize gestures, safe location status, and a typed Options application facade that delegates cadence/adoption/reconciliation to M14-N.1. Manual Export remains independent; no schema, Backup, permission, host, or dependency change is introduced.
- **M14-N.3 — Backup Strategy Simplification & Monthly Reminder (COMPLETE AT `d410ecb`):** cancels the former real-Chrome automatic-backup validation, retires production scheduler/folder/retention behavior, removes `alarms`, preserves Manual Backup v7 and historical import compatibility, records local successful-export time, and derives an advisory recommendation at 30 elapsed days. M14-N.1/N.2 remain historical checkpoints.
- **M14-P.1 — Snippet Delivery Performance Re-measurement & Optimization (COMPLETE / ACCEPTED AT `7bc5005`):** promoted ahead of M14-O for daily-use responsiveness. Stable current Text and 64×64/640×480/1440×900 PNG/JPEG/WebP measurements identify one Category-A bottleneck in native PNG request serialization and improve the 4.46 MiB warm median from 254.3 ms to 1.5 ms without protocol, safety, schema, permission, cache, backup, or destination changes. Categories B/C/D are documented and deliberately not over-optimized. This does not complete M14-P.
- **M14-P.2 — Daily-Use Snippet UX Cleanup (F1 + F2) (COMPLETE / VALIDATED AT `64b5dd8`):** promoted ahead of M14-O. Text-editor safe links are scoped blue/underlined presentation with no general formatting feature; explicit Edit deterministically scrolls the existing form and focuses Text content or Image Title without timers, polling, autofocus, file-picker activation, save/data change, delivery change, or M14-P.1 change. This does not complete M14-P.
- **M14-P.3 — UI/UX Architecture & Roadmap Documentation (COMPLETE AT `406ff08`):** Decision 56 makes the visible daily-use experience authoritative before generated retrieval.
- **M14-P.4/M14-P.4.1/M14-P.4.2 — Navigation & Snippet Library UI Completion (COMPLETE / APPROVED AT `52befe0`):** preserves popup-free toolbar-to-Side-Panel, UI-only Knowledge hiding, completed Snippet hierarchy/actions/confirmation, authoritative clipboard-only Copy, and retained Image preview/fallback. M14-P.4.2 adds the approved responsive Workspace presentation and configures the existing Options application for normal-tab display. M15 still owns all provider/model, generation, Prompt Builder, Context Image ingestion, output lifecycle/Copy, and F3 behavior; permissions, native code, trigger behavior, asset bytes, Dexie v6, and Backup v7 remain unchanged.
- **M14-P.5 — Performance Closure & Roadmap Documentation (COMPLETE / PRINCIPAL-APPROVED AT `d94181746cff07632fba1cdb9c6d874b569b4f21`):** Decision 57 makes M14-P.1 the authoritative performance baseline and records that no second broad/speculative optimization milestone will occur before M14-O. Text remains the highest performance priority; Image quality/correctness/safety and the one-shot native architecture remain accepted. The final M14-P gate owns the next Text benchmark comparison and real-Chrome responsiveness check.
- **M14-P.5.1 — Milestone Transition Correction (DOCUMENTATION ONLY):** corrects repository continuity after the approved M14-P.5 checkpoint and activates M14-O without changing executable behavior or adding an implementation milestone.
- **M14-O — Generated Text Snippet Tags & Retrieval Integration (IMPLEMENTED / APPROVED / CHECKPOINTED AT `6e893886ea23870c374e7e96dcc09a1c92184afc`):** implements version-1 fingerprinted Text-only generated metadata, strict 64-KiB-input/4-KiB-array parsing, provider-neutral `GenerateSnippetTags` with no production adapter, transactional invalidation and stale/concurrent-result protection, explicit maximum-20/concurrency-one backfill, and deterministic generated-tag weight 1. Authored tags remain separate; usage/recency/generated time remain outside ranking; Knowledge/Prompt Builder v1, Image Snippets, UI, delivery, permissions, Dexie v6, and Backup v7 remain unchanged.
- **M14-P.6/M14-P.6.1 / M14-P — Final Snippet Completion Gate (COMPLETE / PRINCIPAL-APPROVED):** complete feature-family and full-suite evidence, production-output/security/schema/dependency audits, delivery/retrieval isolation, and the M14-P.1 Text benchmark comparison pass with no blocker. The Principal accepted the complete 14-item real-Chrome/manual matrix. Text performance and Image quality/safety are accepted, no known blocking Snippet issue remains, and the complete Snippet foundation is stable and complete.

The completed product order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-P.1 performance → M14-P.2 F1/F2 → M14-P.3 documentation → M14-P.4 visible navigation/Library completion → M14-P.5 performance closure documentation → M14-P.5.1 transition correction → M14-O generated retrieval checkpoint → M14-P.6 automated gate → M14-P.6.1 Principal manual closeout → M14-T bounded rendering → M14-U/M14-U.1 performance attribution and closeout. M14-M.4 and M14-R are independent workflow records. M15-A architecture/documentation is complete and awaiting Principal review; M15 runtime remains **NOT STARTED**. F3 remains M15 and canonical activation remains `;trigger + Space`.

## M14-Q — Internal Performance Audit & Accepted Follow-ups

M14-Q is **COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT `5003187`**. [The completed audit](PERFORMANCE_AUDIT_M14-Q.md) preserves evidence/failures, independent UI/storage/lifecycle controls, bounded mixed/near-cap results, repository validation, and Documentation Impact Review. Its valid-large-PNG correctness finding was completed by M14-S at `ccc1930`; bounded Library rendering was completed by M14-T at `50eaf76`; and Backup restore write-path attribution was completed and Principal-approved by M14-U at `b8dfef1`.

M14-U preserves the authoritative conclusion that 10,000-Snippet restore remains expensive: the awaited Snippet IndexedDB write is the dominant observable boundary, current indexes have measurable associated cost, and the fresh-write versus clear/rewrite and transaction-context gap remains unresolved. M14-Q-F2 is **DEFERRED / EVIDENCE-PRESERVED**; no M14-V or other restore diagnostic/optimization is scheduled before M15, and no schema, index, transaction, atomicity, or Backup redesign is approved. F4 and F8 remain **DEFERRED / EVIDENCE-DEPENDENT** future considerations. Approved M14-Q LOCAL ONLY evidence remains intentionally outside Git and untouched.

M14-U.1 records this final performance-follow-up disposition and is checkpointed and pushed at `e9d0f97`. It activates no additional performance implementation. M15-A is now the architecture/documentation entry to the next engineering milestone; M15 runtime remains **NOT STARTED**.

## M14-R — Interruption-Safe Coding-Agent Workflow Standardization

**COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT `f2fe6e6`.** This independent supplemental task establishes repository-based resumability for substantial coding-agent work. The normative `Interruption-Safe Agent Execution` rule defines concise Durable Resume Checkpoints, phase/cost-aware checkpoint cadence, efficient low-capacity behavior, safe-pause and unexpected-recovery protocols, same-task identity across session interruptions, completion cleanup, and required interruption expectations in future substantial Principal task specifications.

`Durable Resume Checkpoint != Git Checkpoint`: working-state documentation does not authorize staging, commit, or push, and no extra Git commit is required merely to survive a session limit. M14-R changes no product milestone, architecture, source, schema, Backup behavior, permissions, native protocol, Snippet/provider behavior, performance implementation, or M15 functionality. At its checkpoint, M15 remained the next unstarted product milestone; M15-A architecture/documentation is now complete awaiting Principal review and M15 runtime remains **NOT STARTED**.

## Milestone 15 — AI Drafting Workflow Refinement and Multimodal Context

### M15-A — AI Workspace Architecture & Contract Lockdown

M15-A is **ARCHITECTURE/DOCUMENTATION COMPLETE / AWAITING PRINCIPAL REVIEW**. M15 runtime implementation remains **NOT STARTED**. Decision 58 and [AI_WORKSPACE_ARCHITECTURE.md](AI_WORKSPACE_ARCHITECTURE.md) lock the implementation-ready contract before executable work begins.

M15 will consume the existing M14-P.4 responsive Side Panel presentation without new navigation. Its versioned replacement contracts provide Text-Snippet-only retrieval, authority-ordered prompt assembly v2, typed transient Context Images, opaque provider-independent model discovery with explicit image capability, immutable one-request-at-a-time generation, preserved editable output, exact edited-output Copy, and a one-use in-memory handoff to existing Text Snippet authoring. Historical M7/M8/M9 v1 contracts, Knowledge compatibility data, Image Snippet delivery, Dexie v6, Backup v7, permissions, and M14-O scoring remain intact.

Only Ollama is implemented in M15. OpenAI, API keys, provider selection, cloud/provider policy, and multi-provider expansion remain M16. The next M15 implementation task requires separate Principal definition and activation after M15-A review/checkpoint.
