...
- **M14-O — Generated Text Snippet Tags & Retrieval Integration (IMPLEMENTED / AUTOMATED VALIDATION PASS / AWAITING PRINCIPAL REVIEW):** implements version-1 fingerprinted Text-only generated metadata, strict 64-KiB-input/4-KiB-array parsing, provider-neutral `GenerateSnippetTags` with no production adapter, transactional invalidation and stale/concurrent-result protection, explicit maximum-20/concurrency-one backfill, and deterministic generated-tag weight 1. Authored tags remain separate; usage/recency/generated time remain outside ranking; Knowledge/Prompt Builder v1, Image Snippets, UI, delivery, permissions, Dexie v6, and Backup v7 remain unchanged.
- **M14-P — Final Snippet Completion Gate:** after M14-O, confirm daily-use UX, Library/navigation, Image editing, generated retrieval, complete regression evidence, synchronized documentation, and appropriate real-world/manual validation. Re-run the established Text diagnostic where appropriate, compare with M14-P.1, validate real-Chrome Text trigger-to-delivery responsiveness in the normal daily-use workflow, and confirm M14-O/later work caused no meaningful regression. Small synthetic differences do not automatically create optimization work; a meaningful user-visible delay must first be localized to its actual stage. M15 begins only after this gate completes.

The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-P.1 performance → M14-P.2 F1/F2 → M14-P.3 documentation → M14-P.4 visible navigation/Library completion → M14-P.5 performance closure documentation → M14-P.5.1 transition correction → M14-O generated retrieval implementation/review → M14-P final completion gate with Text performance regression verification → M15 AI Workspace. M14-M.4 is the intervening documentation-only Git environment record. No second performance milestone is scheduled between M14-P.5 and M14-O. F3 remains M15 and canonical activation remains `;trigger + Space`.

## Milestone 15 — AI Drafting Workflow Refinement and Multimodal Context

After the final M14-P gate, M15 consumes the M14-P.4 responsive Side Panel presentation and activates the actual Decision 46/47 AI workflow: Context/Gist empty-state and grounding behavior; Text-Snippet-only active reference retrieval; a provider-independent Model catalog/application boundary; Generate and provider execution; preserved generated-output lifecycle; functional **Save as Snippet** and **Copy**; and reduced primary-workflow copy. Save as Snippet navigates to Text Snippet authoring with generated response content prefilled, permits user edits to title/trigger/tags/content, and persists only after explicit user Save—generation never auto-creates a Snippet. M15 also implements removable request-scoped Context Images with transient, local-first behavior by default, a provider-independent attachment boundary, capability checks, and explicit unsupported-image feedback. Knowledge compatibility remains non-destructively preserved. Detailed application, provider, persistence/lifetime, migration, and UI architecture remains deferred to future M15 tasks.
