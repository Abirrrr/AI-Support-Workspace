...
- **M14-N.2 — Automatic Backup Options Activation & Production Validation (NEXT):** add the explicit user-gesture folder picker/reauthorization/status UI, call runtime cadence/location reconciliation, and revalidate restart, revocation/unavailability, same/different-folder behavior, missed scheduling, output, and retention through the final production path in real Chrome. Different-folder positive evidence remains pending this gate.
...
The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → M14-N.1 → M14-N.2 → M14-O → M14-P → M15. M14-M.4 is the intervening documentation-only Git environment record and does not alter product sequencing. This decomposition proves the selected-folder premise before permanent schema commitment, records real-world feedback without implementing it, then uses one shared data foundation before feature behavior to avoid unnecessary migrations while keeping usage, automatic backup, and generated tagging independently fail-safe.
