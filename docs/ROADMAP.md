...
M14-U.1 records this final performance-follow-up disposition without activating implementation. After its separately authorized documentation checkpoint, M15 is the next engineering milestone but remains **NOT STARTED** until its first implementation task is explicitly defined and activated.
...
`Durable Resume Checkpoint != Git Checkpoint`: working-state documentation does not authorize staging, commit, or push, and no extra Git commit is required merely to survive a session limit. M14-R changes no product milestone, architecture, source, schema, Backup behavior, permissions, native protocol, Snippet/provider behavior, performance implementation, or M15 functionality. M15 remains the next product milestone and is **NOT STARTED**.

## Milestone 15 — AI Drafting Workflow Refinement and Multimodal Context

### M15 status after performance follow-up closeout

M15 is the **NEXT MILESTONE**, but implementation has **NOT STARTED**. Once the Principal defines and activates its first implementation subtask, M15 will consume the existing M14-P.4 responsive Side Panel presentation and activate the actual Decision 46/47 AI workflow: Merchant Context behavior; Context/Gist empty-state and grounding behavior; Text-Snippet-only active reference retrieval; a provider-independent Model catalog/application boundary; Generate and provider execution; preserved editable Generated Output lifecycle; functional **Save as Snippet** and **Copy Generated Output**; and reduced primary-workflow copy. Save as Snippet navigates to Text Snippet authoring with generated response content prefilled, permits user edits to title/trigger/tags/content, and persists only after explicit user Save—generation never auto-creates a Snippet. M15 also owns removable request-scoped Context Images with transient, local-first behavior by default, a provider-independent attachment boundary, capability checks, and explicit unsupported-image feedback. Knowledge compatibility remains non-destructively preserved. Detailed application, provider, persistence/lifetime, migration, and UI architecture remains deferred to future M15 tasks.
