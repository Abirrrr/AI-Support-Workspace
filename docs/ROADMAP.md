...
- **M14-P — Snippet Hardening Validation & Closeout:** complete automated migration/import/restore/delivery/failure/ranking/privacy coverage, Backup v7/manual-reminder compatibility, F1/F2 daily-use UX corrections, and a Principal review of Text/Image authoring, activation, clipboard-only/automatic delivery, destination compatibility, lifecycle recovery, usage, generated tags/retrieval, import/export, real-world feedback, user-visible performance, regressions, and documentation consistency. Re-measure delivery performance; if meaningful user-visible delay remains, require a focused optimization gate before closeout. M15 begins only after M14-P completes.

The product implementation order is M14-L/L.1 → M14-M.0 → M14-M.1/M14-M.1.1 → M14-M.2 → M14-M.3/M14-M.3.1 → historical M14-N.1/N.2 → M14-N.3 simplification/reminder → M14-P.1 focused performance gate → M14-P.2 F1/F2 daily-use UX cleanup → M14-O → M14-P final completion gate → M15. M14-M.4 is the intervening documentation-only Git environment record. F3 remains M15 and the canonical trigger remains `;`.

## Milestone 15 — AI Drafting Workflow Refinement and Multimodal Context

After the completed M14-P hardening gate, M15 carries Decision 54's primary shell/navigation work: retire the popup/default popup, configure the native toolbar action to open/toggle the global Side Panel, and add the compact accessible Settings gear that opens the existing Options / Libraries page. Then implement the Decision 46/47 AI workflow through small reviewed tasks: compact auto-growing Merchant Context and Guidance / Gist inputs; the exact empty-state/grounding behavior; Text-Snippet-only active reference retrieval; a provider-independent Model dropdown/application boundary; preserved editable output; **Save as Snippet** and **Copy** in the output header; and reduced primary-workflow copy. Save as Snippet navigates to Text Snippet authoring with generated response content prefilled, permits user edits to title/trigger/tags/content, and persists only after explicit user Save—generation never auto-creates a Snippet. Allow one or more removable request-scoped images to be pasted into Merchant Context with transient, local-first behavior by default, a provider-independent attachment boundary, provider capability checks, and explicit feedback when a provider cannot use or omits an attachment. Knowledge compatibility remains dormant and non-destructively preserved. Detailed application, provider, persistence/lifetime, migration, and UI architecture remains deferred to future M15 tasks.
