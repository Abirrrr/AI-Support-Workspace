...
M14-M.1 implements all three stores without rewriting authored Snippet records or indexes: `snippetUsageStats: 'snippetId, lastUsedAt'`, `snippetGeneratedMetadata: 'snippetId, generatedAt'`, and feasibility-proven `automaticBackupState: 'id'`. M14-N.1 extends only the singleton value and treats different/uncertain identity as a new set; positive final production behavior remains subject to M14-N.2 real-Chrome proof. `SnippetEntry.tags` remains the authored ordered array. M14-M.1 wires source-change invalidation, deletion cascade, and restore atomically; generated-tag creation/retrieval remains M14-O.
...
Milestones 3 through 14 are complete. M14-M.0 remains the real-Chrome feasibility PASS, M14-M.1 implements Dexie physical version 6 plus canonical Backup v7, M14-M.3 implements delivery usage counting, and M14-N.1 implements automatic schedule/file output/retention using logical fields in the existing singleton. No schema/Backup version evolution, generated-tag provider call/retrieval scoring, F1/F2, Context Image, or compact Workspace behavior is introduced. The local directory handle and operational state are never backed up; M14-N.2 must still prove the final UI-wired production adapter and different-folder identity in real Chrome.
