...
`automaticBackupState` is the local singleton keyed by physical `id: 'global'`. Its required fields are the structured-clone directory handle, canonical random `backupSetId`, and bounded `managedBackups` array. M14-N.1 adds optional logical schedule (`scheduleId`, `cadence`, `anchorAt`, `nextDueAt`), lease (`leaseId`, bound set/schedule, `acquiredAt`, `expiresAt`), attempt/success/failure status, and verified manifest entries (`backupId`, `backupSetId`, exact filename, `createdAt`, v7 automatic identity, byte length, SHA-256). The record remains in the existing Dexie v6 `automaticBackupState: 'id'` store with no schema/index change. It is machine/profile-local, excluded from Backup v7, and preserved independently across canonical profile restore. M14-N.2 reads it only through the application/repository boundary to derive safe UI status and delegates every mutation to M14-N.1.

M14-N.3 leaves this store physically intact but dormant. Current production startup, UI, and Manual Export do not read or activate it. No migration clears it, and no code enumerates or deletes historical managed files.

M14-N.3 adds local `lastSuccessfulBackupAt` as an optional canonical UTC ISO field on the existing global Settings record through a focused backup-reminder repository. This changes no table, key, index, or Dexie version. Settings saves and atomic restore preserve the existing local value. Backup snapshot mapping omits it, so import cannot create or refresh local backup success.
...
| `automaticBackupState` singleton | structured-clone directory handle, random backup-set ID, bounded verified manifest, optional schedule/status/30-minute lease | No row = no authorized location; missing schedule = no operational scheduled output | Excluded as nonportable environmental state | Preserved independently across restore; M14-N.1 updates operational fields atomically and M14-N.2 supplies explicit UI authority through that boundary |

The managed-file manifest record is local operational metadata containing exact `backupId`, `backupSetId`, `createdAt`, filename, canonical Backup v7/automatic identity, byte length, and SHA-256 digest. Manifest membership itself means closed-file verification succeeded; failed or uncertain writes never enter it. Destination identity is re-proved from the active stored handle/set before deletion, while cadence remains in schedule state rather than each entry. The directory handle itself is never represented as a string path, JSON, or backup field.
...
M14-M.1 implements all three stores without rewriting authored Snippet records or indexes: `snippetUsageStats: 'snippetId, lastUsedAt'`, `snippetGeneratedMetadata: 'snippetId, generatedAt'`, and feasibility-proven `automaticBackupState: 'id'`. M14-N.1/N.2 historically extended and used the singleton; Decision 55 makes it dormant in current production. `SnippetEntry.tags` remains the authored ordered array. M14-M.1 wires source-change invalidation, deletion cascade, and restore atomically; generated-tag creation/retrieval remains M14-O.

M14-O activates the existing generated-metadata lifecycle without changing the physical record, store/index declaration, Dexie version, or frozen Backup v7 DTO. Version support remains the documented version-1 canonical serialization inside the 64-character `sourceFingerprint`; unknown record fields or non-v1/malformed fingerprints are invalid. A new project-owned conditional save takes validated metadata plus its immutable Text source snapshot, independently verifies their SHA-256 relationship, then uses one Dexie transaction over `snippetEntries` and `snippetGeneratedMetadata` to compare current title/content/authored-tag material and write only on an exact match. A mismatch returns false and writes nothing.

Generated metadata remains optional. Retrieval and explicit backfill read through `SnippetGeneratedMetadataRepository`; invalid or unreadable rows fail soft to zero retrieval authority or backfill eligibility and never make an authored Snippet unavailable. Backfill and retrieval introduce no indexes, normalized-token persistence, migration, generated timestamp ordering, usage ordering, or Image metadata. Backup remains v7: v1–v6 restore absence, v7 exact records round-trip, and strict unknown-field/fingerprint/ownership validation remains frozen.

M14-M.3 implements atomic `recordUse(snippetId, usedAt)` in one read/write Dexie transaction covering the Snippet owner and usage sidecar. Missing stats create count 1; existing safe counts increment without lost concurrent updates; `Number.MAX_SAFE_INTEGER` remains saturated while `lastUsedAt` advances. Invalid timestamps, missing owners, or write failures roll back both fields. Delivery callers never read/compute/write the count themselves. Usage writes do not alter `SnippetEntry`, assets, generated metadata, indexes, schema version, or catalog revision/publication.
