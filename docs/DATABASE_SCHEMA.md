...
Principal-approved Decisions 50–53 select one future physical Dexie version 6 migration and strict Backup v7 evolution. This section describes the approved target, not current physical data. M14-M.0 must first pass the real-Chrome selected-folder feasibility gate using scratch persistence; M14-M.1 owns the eventual coordinated implementation only after that PASS.
...
After M14-M.0 passes, the future Dexie v6 declaration adds three stores without rewriting authored Snippet records or indexes: `snippetUsageStats: 'snippetId, lastUsedAt'`, `snippetGeneratedMetadata: 'snippetId, generatedAt'`, and the feasibility-proven `automaticBackupState: 'id'` design. If the gate fails, this declaration is not committed and the architecture returns to Principal review. `SnippetEntry.tags` remains the authored ordered array. Usage/generated writes never touch `SnippetEntry.updatedAt` or publish the trigger catalog. Snippet create/update/delete and restore must coordinate sidecar invalidation/cascade atomically where integrity requires it.
...
Milestones 3 through 14 are complete. M14-G/G.2 remains the frozen Backup v5 foundation. M14-K.2 adds only the Settings paste-mode value and strict Backup v6 described above. M14-L/L.1 is architecture/documentation only: Knowledge remains stored/backed up, and generated tags, usage statistics, automatic-backup cadence/location/retention, Context Images, and compact Workspace state are not currently persisted. No native context, request, authorization, capability, HWND, PID, clipboard sequence, delivery result, directory handle, schedule, or managed-file manifest is currently persisted or backed up. Dexie physical version 5 and Backup v6 remain current; Dexie v6 and Backup v7 are approved future targets gated by M14-M.0, not implemented state.
