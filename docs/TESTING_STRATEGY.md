...
Automated focus assertions also cannot prove Chrome WebContents activation or which surface receives physical keyboard input. First invocation from a closed panel must be checked in real Chrome for immediately usable Guidance focus. Repeated invocation while the panel is already visible must still pass capture, delivery, exact Context replacement, state preservation, and the internal Guidance DOM focus/caret assertions; if the webpage retains browser-level keyboard routing, that is recorded as the approved platform limitation and the user may need to click Guidance.

Production build output may report a cross-world preload mismatch and unused generated preload links. These are non-blocking WXT/Vite/Chrome generated-output warnings: the generated HTML contains the preload behavior, application source does not explicitly request it, and the investigation found no functional impact. They do not justify WXT configuration changes in M10. A separate future investigation is warranted only if functional or performance evidence establishes an impact.
...
Milestone 10 requires real Chrome validation after implementation review. Reload the extension; confirm `capture-selection-to-workspace` and its assigned or user-remapped key in `chrome://extensions/shortcuts`; invoke it from normal document, textarea, and available contenteditable selections; and confirm the corrected capture-first-invocation/immediate-open-invocation sequence physically opens the panel from the keyboard command. Verify exact Context replacement; preserved Guidance/model/output; immediately usable Guidance focus with a collapsed caret and no text selection or replacement when a closed panel opens; the same internal DOM focus/caret request on repeated capture; absence of requested final Merchant Context focus; no automatic generation; and repeated replacement without panel toggle. If the already-visible panel does not receive browser-level keyboard routing, confirm the user can click Guidance and continue; do not treat that Chrome limitation as failure of the DOM focus implementation. Validate that empty selection and a restricted Chrome page preserve safe feedback without forcing Guidance focus, then verify normal Generate, popup Workspace and Library navigation, absence of blocking runtime errors, and the exact least-privilege permission surface. Shortcut capture validation does not require live Ollama; generation regression may be run separately where useful.

The completed focus investigation passed first invocation for panel opening, capture, exact Context replacement, preserved state, immediately usable Guidance focus, and collapsed end-caret placement. Repeated invocation passed panel visibility, capture, delivery, exact Context replacement, preserved state, and the internal focus/caret implementation and automated assertion, while Chrome retained webpage keyboard routing and required a click in Guidance. Automatic browser-level focus is therefore unavailable on that path because of the platform limitation. The initial sequencing pass also proved command registration and Chrome-native remapping to `Ctrl+Shift+Y`, command dispatch, on-demand selection capture, direct Side Panel opening, and repeated success when capture and open were initiated without an intervening await. Remaining M10 functional checks are still pending; the focus limitation is not an M10 blocker once they pass, and M10 is not complete.
