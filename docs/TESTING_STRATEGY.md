...
- Real Chrome on Windows has proved that a Decision 42-safe PNG prepared by the companion pastes as a visible genuine image. One small post-cleanup smoke test remains pending Principal Engineer before checkpoint.
...
M14-G.2 combines the cancelled list-only manual check with Text typing/formatting/list Save/reopen and screenshot Ctrl+V preview/Save/reopen after source review. Real Chrome proves M14-I Text clipboard preparation, trigger cleanup, copied notice, and native-paste fidelity for bold, italic, links, bullets, and numbering. Historical browser Image results remain offscreen `clipboard-write-failed`, M14-I.1.4 `snippet.png`, A1 `TEXT`, B `VISIBLE IMAGE`, and A2 not run. Real Chrome now also proves Settings `Ready` and Windows native Image trigger/preparation/cleanup/notice/visible paste. M14-I.5 cleanup is automated-validated; its small post-cleanup smoke test remains pending. M14-J remains blocked until the M14-I checkpoint.
