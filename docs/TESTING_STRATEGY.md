...
### Decisions 46–48 Future Validation Contract

These are future implementation acceptance requirements, not current tests and not authorization to change runtime behavior in M14-K.4:
...
- Automatic backup tests must prove reuse of the canonical Backup format, local/bounded retention, `Off | Daily | Weekly`, weekly default, manual Export preservation, non-blocking failure, and exact declared permissions/capabilities.
- Generated-tag tests must preserve authored tag order/content, distinguish generated metadata according to the later schema, keep content eligibility independent of missing/wrong tags, and use the provider-independent boundary.
- Usage tests count once only after clipboard success plus exact cleanup in both paste modes, do not inspect physical `Ctrl+V`, retain the count after a later automatic-paste failure, and prove persistence failure cannot block delivery.
- Retrieval ranking tests keep textual relevance primary, generated tags supporting, and usage/recency weak deterministic tie-breakers. Popularity cannot outrank materially more relevant content.
- Every future schema/Backup/permission/dependency change receives its own migration, compatibility, generated-output, and real-browser risk review. Current Dexie v5 and Backup v6 must not be rewritten in place.
