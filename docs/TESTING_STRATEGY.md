...
- Prove recognized Text and Image activation allows the ordinary Space while planning, loads authoritative content only after activation, and performs no destination DOM insertion.
...
- Prove clipboard success precedes cleanup. Exact compare-and-swap cleanup removes only the unchanged trigger plus activation Space in textarea, supported text/search input, and generic contenteditable, restores the caret at the removed range start, and emits one bubbling composed deletion `input` notification.
...
- M14-J.1 adds controlled production-controller fixtures for a standard input, textarea, generic contenteditable, and structured nested rich-editor approximation. They assert successful activation, ordinary activation-Space behavior, exact cleanup, original-editor focus, collapsed cleanup caret, surrounding-content preservation, one deletion notification, and no duplicate/stray content. Native Image failure fixtures assert no cleanup or focus loss.
...
M14-K.1 is documentation-only. M14-K.2 must add deterministic extension and native coverage before any real-destination validation; M14-K.3 then owns the minimal real-browser matrix. Existing clipboard-only tests remain mandatory regressions and must not be rewritten as automatic-mode-only tests.

M14-K.2 automated acceptance:
...
- Prove UI outcomes: `Paste sent` only for full input acceptance and never as proof of destination insertion; every automatic non-success after confirmed clipboard preparation retains `Snippet copied — press Ctrl+V` / `Image copied — press Ctrl+V`; cleanup rejection adds `(trigger unchanged)`. A second activation rejected before its own clipboard write retains trigger plus ordinary Space and reports retry-later delivery busy without claiming copied. Clipboard is never cleared, including after automatic success. Low-level Windows errors and payload content do not reach UI/logs.
- Instrument bounded phase timings for Text/Image clipboard preparation, browser/native safety checks, cleanup, and paste result without logging content. Image performance remains non-blocking; M14-K.2 records whether automatic checks add meaningful latency but does not optimize the Image pipeline.
...
Windows cannot atomically identify the browser's internal editor at the exact native input instant. M14-K.3 must stress rapid same-window editor switching at the final boundary. Any observed wrong-editor paste blocks automatic-mode release and requires architecture review; elapsed-time tuning alone is not an acceptable correction.
