...
M13-B.2 automated validation does not substitute for real Chrome proof. After Principal Engineer source review, the rebuilt and reloaded extension must be validated on Intercom and representative normal HTTP/HTTPS pages. Until that occurs, reports must state that manual Chrome validation was not performed and must not claim the browser defect is fixed.
