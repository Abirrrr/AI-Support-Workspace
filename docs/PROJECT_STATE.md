...
- Active milestone: **M15 — AI Workspace**. Active task: **M15-B — Drafting Retrieval & Prompt v2 Foundation**. Status: **IMPLEMENTATION COMPLETE / AWAITING PRINCIPAL REVIEW** as of 2026-09-13. M15-A is **COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT `8aeab99`**. M15 runtime implementation begins only within this bounded provider-independent retrieval and Prompt v2 foundation; later M15 runtime, UI, Context Image, model-discovery, provider, generation, Copy, and Save-as-Snippet behavior remains **NOT IMPLEMENTED**.
...
- Status: **COMPLETE / PRINCIPAL-APPROVED**. M14-P.6 automated validation passed with no blocker, and the Principal accepted the complete 14-item real-Chrome/manual matrix. The complete Snippet foundation is stable and complete. M15-A is complete, Principal-approved, checkpointed, and pushed at `8aeab99`; M15-B is the active bounded runtime foundation.

## Task State

### M15-B — Drafting Retrieval & Prompt v2 Foundation

M15-B implements only the provider-independent Text-Snippet drafting-reference retrieval, lexical query construction, and Prompt v2 application contracts authorized by Decision 58. Historical Retrieval Engine and Prompt Builder v1 behavior remains supported. Knowledge compatibility data remains preserved but is excluded from the new drafting path; Image Snippets remain excluded. No UI, Context Image, model-discovery, provider, generation, Copy, Save-as-Snippet, persistence, schema, Backup, permission, or dependency behavior is in scope.

#### M15-B Durable Resume Checkpoint

```text
Task: M15-B — Drafting Retrieval & Prompt v2 Foundation
Status: IMPLEMENTATION COMPLETE / AWAITING PRINCIPAL REVIEW
Starting branch/HEAD: master at 8aeab99732565d4e3c85f96a99b6c6f3ac114234
Fetched origin/master: 8aeab99732565d4e3c85f96a99b6c6f3ac114234; ahead/behind 0/0
Starting tracked tree/index: clean
Approved M14-Q LOCAL ONLY evidence: present as the documented untracked result set; do not modify, stage, delete, clean, move, normalize, or include
Continuity result: PASS; M15-A is complete, Principal-approved, checkpointed, and pushed at 8aeab99; M15-B is the next sequential task and was not previously assigned
Completed phase: continuity gate and required review; authoritative shared Text Snippet scoring/ranking extraction; historical RetrievalEngine adoption; DraftingSnippetReference and DraftingReferenceRetriever; pure lexical query helper; DraftingPromptBuilderV2; focused M15-B tests; historical RetrievalEngine and Prompt Builder v1 regressions; source/diff review; full automated validation; documentation synchronization and impact review; final integrity and intended-file review
Files changed: docs/PROJECT_STATE.md; docs/CHANGELOG.md; docs/BACKLOG.md; src/application/retrieval/retrieval-engine.ts; src/application/retrieval/text-snippet-ranking.ts; src/application/retrieval/drafting-reference-retriever.ts; src/application/retrieval/drafting-retrieval-query.ts; src/application/prompt/drafting-prompt-builder-v2.ts; tests/unit/drafting-reference-retriever.test.ts; tests/unit/drafting-retrieval-query.test.ts; tests/unit/drafting-prompt-builder-v2.test.ts
Validation completed: focused M15-B plus historical retrieval/Prompt v1 regressions PASS — 6 files / 76 tests; full Vitest final rerun PASS — 72 files / 934 tests, with one opt-in file/test skipped; full ESLint PASS; TypeScript PASS; production build and output validation PASS; all 11 intended M15-B files Prettier PASS; git diff --check PASS; git fsck --full exit 0 with only existing dangling objects and no corruption; all 39 approved M14-Q LOCAL ONLY files retain exact approved bytes and SHA-256 hashes
Isolated pre-existing validation behavior: the first full Vitest run under concurrent validation timed out in the unchanged exact-5-MiB Backup v7 test, which passed alone in 1.83 seconds; the next full run exposed the unchanged Import/Export focus-order flake, which passed alone, and the final full suite passed. Repository-wide Prettier reports only four pre-existing preserved M14-Q evidence Markdown files; they were not modified, while every intended M15-B file passes formatting.
Documentation Impact Review: Project State, Changelog, and Backlog updated. Testing Strategy already contains the Decision 58 durable verification contract and requires no change. AI Workspace Architecture, Architecture, Decisions, Product Requirements, Roadmap, Database Schema, and UI Workflow were reviewed and require no change because M15-B implements the locked contract without redesign.
Incomplete work: Principal review only
Exact next action: Principal reviews M15-B; do not stage, commit, push, or begin later M15 runtime/UI/provider work without separate authorization
Git state: 11 intended unstaged M15-B files alongside untouched approved M14-Q LOCAL ONLY evidence; index empty; staged NONE; commit NONE; push NONE
Architectural concerns: none
Known limitations: M15-B is foundation only and is not wired to the Side Panel, Context Images, model discovery, a generation provider, Generate, Copy, or Save as Snippet
Principal-readiness self-check: PASS
```

### M15-A — AI Workspace Architecture & Contract Lockdown

M15-A is complete, Principal-approved, checkpointed, and pushed at `8aeab99`. It is architecture/documentation only and reconciles the frozen M7 Prompt Builder v1, M8 `GenerationProvider` v1, M9 `OutputWorkflow`/Workspace v1, M11 default-model Settings, M14-O Text-only generated-tag retrieval, and M14-P.4 Side Panel presentation with the approved M15 drafting, transient Context Image, model-discovery, multimodal request, output, Copy, and Save-as-Snippet handoff contracts. It changed no runtime, tests, dependencies, manifest, permissions, persistence, schema, Backup, Knowledge compatibility, M14-Q evidence, or M16 behavior.

#### M15-A Resume State

```text
Task: M15-A — AI Workspace Architecture & Contract Lockdown
Status: COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT 8aeab99732565d4e3c85f96a99b6c6f3ac114234
Starting branch/HEAD: master at e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412
Fetched origin/master: e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412; ahead/behind 0/0
Starting tracked tree/index: clean
Approved M14-Q LOCAL ONLY evidence: all 39 files present with exact approved bytes and SHA-256 hashes; do not modify, stage, delete, clean, move, or normalize
Documents reviewed: PROJECT_STATE; CODING_AGENT_RULES; ARCHITECTURE; DECISIONS; ENGINEERING_PRINCIPLES; PRODUCT_VISION; PRODUCT_REQUIREMENTS; ROADMAP; UI_WORKFLOW; TESTING_STRATEGY; DATABASE_SCHEMA; BACKLOG; CHANGELOG
Implementation reviewed: M7 Prompt Builder v1; M8 GenerationProvider/Ollama adapter; M9 OutputWorkflow and legacy Workspace view; M11 Settings/bootstrap; M14-O Text retrieval/generated tags; M14-P.4 current WorkspaceShell; runtime messaging/Options navigation patterns; WXT manifest permissions
Decisions recorded: Decision 58 and docs/AI_WORKSPACE_ARCHITECTURE.md lock the versioned Text-only retrieval, prompt v2, transient bounded Context Image, opaque model discovery/capability, generation snapshot/lifecycle, exact Copy, and random-token one-use in-memory Save-as-Snippet handoff contracts; no blocking platform conflict identified for same-extension BroadcastChannel handoff
Completed phase: continuity gate; complete canonical-document and implementation review; normative architecture and Decision 58; requirements/roadmap/workflow/testing/backlog/changelog synchronization; README current-status correction; cross-document conflict and link review; formatting and Git integrity validation
Files changed: README.md; docs/AI_WORKSPACE_ARCHITECTURE.md; docs/PROJECT_STATE.md; docs/ARCHITECTURE.md; docs/DECISIONS.md; docs/PRODUCT_REQUIREMENTS.md; docs/ROADMAP.md; docs/UI_WORKFLOW.md; docs/TESTING_STRATEGY.md; docs/BACKLOG.md; docs/CHANGELOG.md
Unresolved conflict: none
Validation completed: cross-document architecture consistency and historical-contract conflict search PASS; changed-document Markdown links PASS; explicit Prettier check for all 11 changed documents PASS; git diff --check PASS; git fsck --full exit 0 with only existing dangling objects and no corruption; branch master at e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412 equals origin/master with ahead/behind 0/0; all 39 approved M14-Q LOCAL ONLY files retain exact approved bytes and SHA-256 hashes
Documentation Impact Review: normative AI Workspace architecture added; Project State, Architecture, Decisions, Product Requirements, Roadmap, UI Workflow, Testing Strategy, Backlog, Changelog, and README synchronized; Coding Agent Rules, Engineering Principles, Product Vision, Database Schema, and Performance Audit reviewed and unchanged; no production, test, performance, dependency, manifest, permission, persistence, schema, Backup, Knowledge, or M16 change required
Exact next action: M15-A is closed at 8aeab99732565d4e3c85f96a99b6c6f3ac114234; M15-B is the active bounded implementation task
Git state: checkpointed and pushed at 8aeab99732565d4e3c85f96a99b6c6f3ac114234; approved M14-Q LOCAL ONLY evidence remains present and untouched
M15 runtime implementation at this checkpoint: NOT STARTED
Principal-readiness self-check: PASS
```

### M14-U.1 — Final Approval and Performance Follow-up Closeout

M14-U.1 is documentation-only. It records the Principal's acceptance of M14-U at synchronized checkpoint `b8dfef1` and closes M14-Q-F2 as **DEFERRED / EVIDENCE-PRESERVED**. No M14-V restore task or other Backup restore performance diagnostic/optimization is scheduled before M15. M14-Q-F4 and F8 remain deferred, evidence-dependent future considerations. M14-U.1 is complete, Principal-approved, checkpointed, and pushed at `e9d0f97`; M15-A architecture/documentation is complete awaiting Principal review while M15 runtime remains **NOT STARTED**.

#### M14-U.1 Final Closeout State

```text
Task: M14-U.1 — Final Approval and Performance Follow-up Closeout
Status: COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412
...
Exact next action: M14-U.1 is closed at e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412; M15-A is the active architecture task
Git state: checkpointed and pushed at e9d0f97e5d7b263b1030a1dcc139ea4ed62f8412; approved M14-Q LOCAL ONLY evidence remains present and untouched
...
M14-P.1 records that Text planning/serialization is effectively instantaneous relative to normal human interaction; its historical warm behavior was below practical timer resolution with p95 around 0.1 ms. Guarded PNG preparation is efficient. JPEG/WebP genuine decode and PNG re-encode are slower but accepted because Image quality, correctness, and safety outrank absolute latency. Image speed work may not resize, downsample, reduce resolution/quality, bypass genuine decode, or weaken Decision 42. Native request serialization received the proven material optimization. One-shot native startup remains measurable but does not currently authorize a persistent host, service, daemon, keepalive, or protocol expansion.
...
The application layer exposes a provider-neutral `GenerateSnippetTags` raw-response port and deterministic service. M14-O adds no production adapter, provider/model call, automatic generation-on-save wiring, startup work, background scheduler, or UI. Generation input is exact semantic Text material capped at 64 KiB UTF-8. The entire raw response is capped at 4 KiB and must be one JSON string array; normalization is NFKC, trim, internal-whitespace collapse, locale-independent lowercase, strict 1–40 code-point validation, control/newline/markup rejection, first-order deduplication, and maximum eight tags. The explicit backfill selects at most 20 eligible Text Snippets by `createdAt` then ID, skips valid current metadata and Image Snippets, runs concurrency one, supports cancellation, and isolates item failure.

Retrieval optionally reads the sidecar in its existing local application path and validates every candidate against the current version-1 fingerprint before scoring. Per unique query token, title is 5, authored tags 3, content 1, and valid generated tags 1. Duplicate generated tokens cannot multiply authority. Missing, stale, unsupported, malformed, or unreadable metadata contributes zero and cannot block title/authored/content retrieval. Usage count, last-used time, and generated time never affect score or tie-breaking. Knowledge and Prompt Builder v1 remain compatible; Image Snippets remain excluded.
...
M14-P.4 owns the approved toolbar action → global Side Panel transition, retirement of the popup/default popup, the small accessible Settings gear → existing Options/management path, and the actual responsive AI Workspace presentation/shell. It renders Merchant Context, structural Context Images, Guidance / Gist, provider-neutral Model/Generate presentation, and editable Generated Output with Save as Snippet/Copy. M15 retains all functional AI behavior: image ingestion, provider-independent model discovery and selection, Prompt Builder integration, generation/provider calls, output lifecycle/Copy, transient handoff semantics, and F3 Save as Snippet. Provider-dependent controls remain honestly disabled in M14-P.4.2.
...
M14-P.4.1 defines `CopySnippetToClipboard`: stable Snippet ID → authoritative Snippet/asset load → shared established Text/Image clipboard preparation and writer → typed result. Library Copy is explicit clipboard-only behavior, never fabricates trigger/catalog/editor state, never invokes Automatic Paste, never creates usage receipts or increments usage, and never uses Image preview bytes as authority.

Implementation retires the popup/default popup, preserves a popup-free toolbar action and `side_panel.default_path`, and enables Chrome's native toolbar-action Side Panel behavior. The production panel includes the approved compact Workspace presentation while functional AI behavior remains M15. The existing Options application is configured for a normal browser tab rather than Chrome's embedded dialog, without a duplicate application or new permission. Options exposes Snippet Library, Settings, and Import / Export while Knowledge code/data/store/tests and Backup v1–v7 remain intact. Snippet items use separate Type/Usage chips, bounded Details, Delete → Edit → Copy accessible icons, stable-ID confirmation deletion, and intentional Image fallback. The Image defect was premature object-URL revocation during React StrictMode effect replay; URL ownership now follows the active effect lifecycle.

### M14-P.2 — Daily-Use Snippet UX Cleanup (F1 + F2)

```text
Active task: M14-P.2 — Daily-Use Snippet UX Cleanup (F1 + F2)
Starting checkpoint: 7bc5005 — perf: optimize snippet delivery serialization
Starting tree: clean; master synchronized with origin/master; git fsck acceptable

F1: safe links blue + underlined inside Text content editor (IMPLEMENTED)
F1 general color/underline controls: NONE
F2 Text target: existing Tiptap content editor at document start
F2 Image target: existing Title input
Scroll: existing authoring form scrollIntoView, behavior auto
Lifecycle: explicit Edit request + React layout/effect lifecycle
Timers/polling/autofocus/automatic save: NONE
...
F1 is presentation-layer-only. The constrained Tiptap content root applies blue and underline utilities to descendant anchors; ordinary editor text and anchors elsewhere in the application are unaffected. The existing safe-link mark, HTTP/HTTPS/mailto validation, persistence mapping, serialization, sanitization, and clipboard output are unchanged. Save and reopen reconstruct the same safe link and receive the same editor-scoped presentation. No text-color command, underline command, toolbar expansion, stored style, or global anchor selector exists.

F2 reuses the single unified Text/Image authoring form. Each explicit Edit action creates a one-use navigation request. After React renders the matching form, a layout effect scrolls that form with deterministic `behavior: 'auto'`; Image Edit focuses the Title input immediately. Text Edit passes the request to the mounted Tiptap adapter, which focuses the content start once the real editor instance exists and suppresses Tiptap's redundant selection scroll because the form owns scrolling. Repeated same-Snippet and A→B edits issue fresh requests. Page load, data load, filtering, new authoring, persistence, and unrelated state changes create no request and move no focus.

The existing unsaved-edit transition behavior is unchanged: choosing another Edit replaces the current draft according to the pre-existing unified state flow, with no new guard or automatic save. Existing Image assets remain intact unless the user explicitly replaces/removes and saves them. Legacy unsupported Rich content continues through its read-only compatibility surface, which is scrolled into view and focuses its Close control after explicit Edit. M14-P.1 source, Text/Image delivery, native serialization/protocol, Decision 42/45, usage receipts, `;trigger + Space`, image quality, M14-O, F3, and M15 are untouched.
...
```text
Active task: M14-N.1 — Automatic Backup Runtime Core
Starting checkpoint: 9b0862c — docs: record separate git metadata workflow
Starting tree: clean; master synchronized with origin/master; git fsck acceptable

Dexie physical version: 6 (UNCHANGED)
Canonical Backup version: 7 (UNCHANGED)
Schedule: one stable named one-shot chrome.alarms alarm
Cadence: Off | Daily 24h/latest 7 | Weekly 168h/latest 4
Execution safety: in-memory guard + atomic persisted 30-minute lease
...
- Status: Milestone 14 and the complete post-M14 Snippet Hardening foundation are complete. M14-P is **COMPLETE / PRINCIPAL-APPROVED**; M14-T, M14-U, and M14-U.1 are complete and checkpointed through `e9d0f97`. M15-A is **COMPLETE / PRINCIPAL-APPROVED / CHECKPOINTED AND PUSHED AT `8aeab99`**. M15-B is the active bounded retrieval and Prompt v2 foundation; later M15 runtime behavior remains unimplemented.
...
- **M15-B — Drafting Retrieval & Prompt v2 Foundation:** IMPLEMENTATION COMPLETE / AWAITING PRINCIPAL REVIEW. The new Text-Snippet-only reference path shares authoritative M14-O scoring with historical retrieval, excludes Knowledge and Image Snippets, and adds the pure Context/Gist query helper plus provider-independent Prompt v2. Later M15 runtime/UI/provider behavior remains unimplemented.
...
- Principal reviews and authorizes the M15-A documentation checkpoint. After that checkpoint, Principal defines and activates the first M15 implementation subtask; no M15 source behavior is active yet.
...
- The Options management surface exposes Snippet Library, Settings, and Import / Export while Knowledge remains hidden but preserved for compatibility. The popup is retired; the toolbar opens the global Side Panel directly, whose Settings gear opens the existing Options application in a normal tab.
...
- The historical project-owned `GenerationProvider` v1 and local-only `OllamaProvider` v1 exist and remain unchanged. Decision 58 defines M15's versioned replacement application contracts; OpenAI Provider Expansion remains M16, and supported future activation of an already-visible Chrome Side Panel remains an approved unassigned direction.
- M14-O adds the narrower provider-neutral `GenerateSnippetTags` application port without a production adapter. Existing provider code is unchanged and is not invoked by generated-tag production runtime. No automatic save/startup/backfill wiring exists.
...
- Current completed checkpoint: M15-A is complete, Principal-approved, checkpointed, and pushed at `8aeab99`. M14-U.1 is complete at `e9d0f97`, M14-U at `b8dfef1`, M14-T at `50eaf76`, and M14-O at `6e893886ea23870c374e7e96dcc09a1c92184afc`; M14-P is complete and Principal-approved. M15-B is active over the synchronized M15-A base.
...
- Exact next Principal action: review M15-B implementation and validation. Do not stage, commit, push, or begin later M15 runtime/UI/provider work without separate authorization.
...
- Historical Image Snippet latency concern was re-measured and addressed by approved M14-P.1 checkpoint `7bc5005`; subsequent daily-use validation accepted the optimized delivery path. M14-O and later Snippet work must keep delivery outside generated-metadata/retrieval prerequisites. Only a measured meaningful user-visible regression at the final M14-P gate may open a focused optimization task for the proven stage before M15.
