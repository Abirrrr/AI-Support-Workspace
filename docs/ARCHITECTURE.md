...
The platform architecture remains approved: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, Tiptap v3 as constrained UI infrastructure, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. The optional Windows infrastructure boundary uses C#/.NET 10 as fixed by Decision 43. Milestones 1 through 13 are implemented and validated. M14 is current under Decisions 36–43. M14-G/G.1/G.2/G.2.1 is committed at `672185e`; M14-F inline Rich-image authoring remains cancelled. M14-I through M14-I.5 is complete, committed, and pushed at `ebe915f`. Text clipboard/native-paste, Windows native Image delivery, and the post-cleanup smoke test are real-Chrome validated. Browser Image experiments and probes are removed from active runtime and preserved only as historical evidence. Decisions 42 and 43 are unchanged. M14-J is next but has not started.
