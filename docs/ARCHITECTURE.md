...
Milestone 8 defines one narrow project-owned `GenerationProvider` boundary. Application consumers depend on this contract rather than Ollama, HTTP response types, or provider SDKs. A provider exposes stable project-owned identity and a `generate` operation that accepts a transient `GenerationRequest`, optionally receives an `AbortSignal`, and returns a provider-independent `GenerationResult`. `OllamaProvider` is the first infrastructure implementation and uses provider identity `ollama`; no provider registry or placeholder for a future provider is introduced.
...
The platform architecture remains approved and frozen: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. The Milestone 1 development foundation, Milestone 2 runtime extension shell, Milestone 3 local persistence foundation, Milestone 4 Knowledge Library, Milestone 5 Snippet Library, Milestone 6 Retrieval Engine, and Milestone 7 Prompt Builder are implemented. The headless Prompt Builder consumes optional Merchant Context, optional Guidance, and optional prepared Retrieval Results, then deterministically returns the frozen typed provider-independent `PromptAssembly` without invoking retrieval, persistence, browser, UI, or provider boundaries. Milestone 7 passed Principal Engineer review and comprehensive deterministic validation without architecture, schema, browser-surface, provider, dependency, or permission changes. No M7-specific manual Chrome validation was required because the boundary is headless, has no browser interaction, and is comprehensively covered by automated tests. Milestone 7 implementation checkpoint `a71dfed` is committed, pushed, and synchronized. Milestone 8 — Ollama Provider is current; its provider architecture is implementation-ready, but the provider and its runtime integration are not implemented.
