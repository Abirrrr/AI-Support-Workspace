...
Decision 40 approves a future shell correction without changing the current implementation. The extension toolbar action will open or show the existing global AI Support Workspace Side Panel directly, with no intermediate popup. Chrome's supported mechanism is `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })`. The existing `sidePanel` permission is sufficient for that behavior; no new permission is expected solely for this correction.

The Side Panel remains the focused daily Workspace rather than becoming the full management application. A Library action inside the panel will open the existing full options/Library page in a normal browser tab. Knowledge, Snippets, Settings, Import / Export, and future management surfaces remain owned by the options page and are not moved into the Side Panel.

The future implementation must inspect WXT's generated manifest and action configuration, remove or retire `action.default_popup` and the default popup entry point cleanly, and prove that popup and direct-side-panel action behaviors do not compete. It must preserve the existing `side_panel.default_path`, global panel scope, keyboard shortcut behavior, options-page boundaries, and least-privilege permission set. This work is an unassigned future Workspace Shell/extension-action UX item; it is not part of M14-G, M14-H, M14-I, or M14-J and does not interrupt the approved Snippet sequence.
...
Clipboard transport uses explicitly granted optional `clipboardWrite` and `offscreen`, one packaged static `CLIPBOARD` document, bounded request-scoped JSON-compatible binary encoding, one serialized write at a time, and immediate lifecycle closure. `clipboardRead` and synthetic paste remain prohibited.
...
Clipboard success precedes exact trigger-plus-activation-Space cleanup. Changed text, selection, root, catalog identity, or request identity skips cleanup and cannot overwrite user edits; the prepared clipboard remains usable through native Ctrl+V.
...
M13 trigger-recognition and publication guarantees remain frozen: canonical semicolon lookup; trusted cancelable Space `beforeinput`; inactive composition; collapsed caret; trigger immediately before the caret with start/whitespace left boundary; fail-safe normal typing; ordered invalidation; and the publication barrier. M14-I supersedes direct insertion for deliverable entries: activation Space proceeds during clipboard preparation, and confirmed success permits compare-and-swap removal of only the unchanged trigger plus that Space. Colon activation is not M14 scope.

#### Transient Catalog Continuity

Dexie remains the sole persistent source. The service worker derives the implemented metadata-only typed catalog, and content scripts never access Dexie. Each exact validated entry contains only delivery kind, canonical trigger, and Snippet ID. It contains no deterministic plain projection, Rich structure, generated HTML, Blob, base64, asset ID, filename, MIME type, image bytes, surrounding editor text, host data, history, provider state, logs, or unrelated Library records. Authoritative content and assets are loaded only after activation.
...
Because planning and clipboard writing are asynchronous while `beforeinput` is synchronous, ordinary Space proceeds during preparation. The service worker checks the activation catalog identity before authoritative planning and again immediately after planning before clipboard mutation. Confirmed write then permits exact content-script compare-and-swap cleanup only if the same editor, trigger, activation Space, selection contract, catalog epoch/revision, and request remain unchanged. Otherwise content remains untouched; a post-copy stale state reports copied without cleanup, while a pre-write stale state never modifies the clipboard. Real Chrome has proved this flow for Text and Windows native Image delivery.
...
Image delivery means preparing `image/png` on the system clipboard, then asking the user to press real `Ctrl+V`. Stored JPEG/WebP input is decoded and converted to PNG because PNG is the portable mandatory/common clipboard image representation; destination placement remains destination-owned. No synthetic paste or upload reverse engineering is allowed. Copy failure or permission denial preserves the trigger. After success, compare-and-swap cleanup removes exactly the unchanged trigger plus activation space, leaves no placeholder/trailing space, and collapses the caret at the removed range start.
...
The platform architecture remains approved: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, Tiptap v3 as constrained UI infrastructure, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. M14-I is complete at `ebe915f`. M14-J.2 is real-Crisp Text PASS. M14-J.3 extends only trusted activation-time editor resolution: the normal target/Selection path remains primary, while a retargeted Shadow DOM event may resolve the nearest supported internal editor from `composedPath()`, validate and convert one collapsed `getTargetRanges()` range inside it, and use the current composed selection for async cleanup revalidation. It does not query `host.shadowRoot`, change trigger syntax, or alter clipboard ownership; real Intercom activation and ordinary rich Text pass. M14-J.4 leaves canonical direct `ul`/`ol` → `li` serialization intact and corrects supported inline hard breaks to `<br>` only at the safe clipboard HTML boundary. M14-J.5's corrected three-item real record passes persistence, exact serialization, delivery-payload equality, and normal Intercom paste; the initial apparent failure was an invalid two-item fixture. Crisp and Intercom Image paste pass, with Intercom bullet triggering after Shift+Enter retained as a known low-priority compatibility limitation. The established `native-dev` one-shot diagnostic remains explicit, local, read-only, and absent from ordinary production output. M14-J.6 adds idempotent all-frame recovery and interaction-driven worker reconnection under final Decision 44 exact persistent HTTP/HTTPS host access. Principal real-Chrome validation confirms Intercom and Crisp recovery without webpage refresh plus repeated-reload duplicate safety. M14-J is complete. Persisted `SnippetContent`, deterministic plain text, transport, Decisions 42 and 43, native architecture, Dexie v5, and Backup v5 remain unchanged. Manual native `Ctrl+V` remains current behavior; automatic paste belongs to unstarted M14-K.
