...
M14-P.6 is the audit-only gate over the approved Snippet architecture. Source/import inspection confirms M14-O remains absent from trigger delivery and Library Copy; the established Text benchmark remains in the M14-P.1 effectively-instantaneous class; Decision 42 Image bounds/decode/quality, Decision 45 clipboard-first optional Automatic Paste, repository-owned delete/sidecar cleanup, Manual Backup v7 plus reminder, and Dexie v6 remain intact. Production output retains the popup-free Side Panel, normal-tab Options, hidden Knowledge navigation, disabled M15 controls, approved permissions, and no automatic-backup or generated-tag runtime. The automated audit passed with no blocker, and M14-P.6.1 records the Principal-accepted 14-item real-Chrome/manual validation PASS. M14-P is **COMPLETE / PRINCIPAL-APPROVED**. M15-A now defines architecture only; M15 runtime remains not started.

### Approved Future AI Drafting and Snippet-Hardening Architecture

#### M15-A Normative AI Workspace Architecture

M15-A architecture/documentation is complete and awaiting Principal review; M15 runtime remains **NOT STARTED**. [AI_WORKSPACE_ARCHITECTURE.md](AI_WORKSPACE_ARCHITECTURE.md) is the normative implementation contract and Decision 58 records its authority.

M15 wires the existing M14-P.4 Side Panel shell without changing its order or adding navigation. It introduces versioned application contracts rather than mutating M7 Prompt Builder v1, M8 `GenerationProvider`/Ollama v1, or M9 `OutputWorkflow` v1. The active reference path retrieves only Text Snippets with the M14-O deterministic weights and no usage/recency signal; Knowledge remains preserved compatibility data and Image Snippets remain delivery-only. Prompt assembly v2 contains application instructions, Guidance / Gist, Merchant Context text, and bounded Text Snippet references. Context Images remain typed request attachments outside prompt sections.

Context Images are original-byte PNG/JPEG/WebP values held only in mounted Side Panel memory. Paste and explicit local-file selection are the only acquisition routes. Exact fail-closed count, encoded-size, dimension, decoded-pixel, RGBA-allocation, and overflow requirements live in the normative document. The provider-independent model catalog reports `supported | unsupported | unknown` image capability; image requests require exactly `supported` and never silently degrade. Only Ollama is adapted in M15, while Workspace/application logic treats model IDs as opaque and has no provider branch or selector.

The generation workflow snapshots Context, images, Gist, model, and retrieved references immutably, permits only one active request, preserves inputs and prior output during regeneration, replaces output only on success, and reports failure separately. Copy uses exact edited plain text with no delivery/persistence side effect. Save as Snippet uses a random, expiring, one-use, in-memory same-extension `BroadcastChannel` handoff to existing Text authoring. The Options URL may carry only the random token, never output text; no durable handoff storage exists, and only explicit existing Save persists.
