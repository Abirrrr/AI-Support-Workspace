...
Plain Snippets, Rich Snippets containing only portable text blocks, and Rich Snippets containing legacy URL Image References retain deterministic text catalog behavior. The excluded legacy Rich local-image record remains available to the Library, persistence, Backup v4, Retrieval Engine, and Prompt Builder. M14-E implements this narrow filter without changing the M13-B.1 publication barrier. Decision 39 separately requires future Image Snippets to remain omitted until M14-I publishes typed image descriptors; this does not reinterpret Decision 38.
...
Rich authoring state is extension-owned structured data. A controlled contenteditable may be used as an interaction surface, but its HTML is never persisted or trusted as domain state. Target authoring covers ordered paragraphs, bold, italic, links, unordered lists, and ordered lists. Legacy URL references and M14-E local-image blocks remain preservable compatibility data but are not the primary new-image workflow. Block ordering must be keyboard-accessible and cannot require drag-and-drop. No third-party rich-text editor dependency is approved; demonstrated need requires dependency and architecture review before addition.
...
Because clipboard writing is asynchronous while canceling `beforeinput` is synchronous, trigger cleanup uses an exact compare-and-swap after confirmed write: ordinary Space proceeds, and cleanup occurs only if the same editor, trigger, activation space, selection contract, catalog epoch/revision, and request are unchanged. Otherwise the content remains untouched and the UI reports copied without cleanup. Decision 37 originally assigned this proof to the then-planned M14-G. Under Decision 39, M14-I must prove user activation, optional permission, offscreen lifecycle, clipboard representation, and this race behavior in real Chrome; an explicit extension UI Copy action is the safe fallback if trigger-originated writing cannot be made reliable.
...
Dexie remains version 5 because the JSON content shape is not indexed and no store/index changes. Backup v4 remains frozen/importable; exact list and Image Snippet discriminants require Backup v5. M14-G introduces lists, the minimal image discriminant needed to freeze one v5 contract, and Backup v5 together; M14-H adds Image Snippet authoring. This prevents unnecessary consecutive public backup versions.
...
The future catalog is typed: text entries carry their plain delivery text, while image entries carry only trigger, Snippet ID, and an image delivery discriminator. Blob, base64, asset ID, and filename never enter frame snapshots. Upon activation the service worker reloads and validates the one owned asset from Dexie, then sends that requested Blob to a short-lived offscreen clipboard transport. Image entries remain separately excluded until M14-I implements this path.
...
Clipboard capability is opt-in. Current Chrome documentation identifies `clipboardWrite` as warning-bearing, supports runtime optional permissions for permissions outside its exception list, and requires `offscreen` plus a `CLIPBOARD` reason for a packaged MV3 offscreen document. M14-F.1 adds no permission. M14-I must prove WXT optional-permission generation, runtime grant, service-worker/offscreen lifecycle, PNG conversion and pixel/memory safety, user-activation behavior, and real destination paste in the minimum supported Chrome. `clipboardRead` is prohibited.

The revised implementation sequence is M14-G Rich Snippet Structured Lists and Backup v5 Foundation; M14-H Image Snippet Domain Completion and Authoring; M14-I Typed Trigger Delivery Planner and Clipboard Image Delivery; and M14-J Rich Text Delivery and Destination Capability Validation. Rich delivery covers paragraphs, marks, links, and lists only.
...
The platform architecture remains approved and frozen: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. Milestones 1 through 13 are implemented and validated. M13 is complete at implementation checkpoint `b76fcb4` and closeout checkpoint `9a3c7ef`. M14 is current under Decision 36, Decision 37's implemented asset foundation, Decision 38's legacy fail-closed rule, and Decision 39's product correction. M14-E is complete at `1828f09`; M14-F inline Rich-image authoring is cancelled before implementation. No implementation task is active. The next engineering action is the revised M14-G — Rich Snippet Structured Lists and Backup v5 Foundation after Principal approval of M14-F.1.
