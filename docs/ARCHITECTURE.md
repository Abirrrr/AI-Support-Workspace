...
Snippet hardening is future best-effort metadata/infrastructure. Periodic local backup reuses the canonical Backup format with `Off | Daily | Weekly`, weekly recommended, bounded retention, manual Export preserved, and explicit permission review. Generated Text Snippet tags coexist with rather than overwrite current authored ordered tags and remain non-authoritative retrieval hints. Usage statistics count after clipboard success plus exact cleanup, do not attempt to observe physical paste, and should remain separate mutable metadata that cannot block delivery. Text relevance stays primary; tags support it; usage/recency are weak tie-breakers.
