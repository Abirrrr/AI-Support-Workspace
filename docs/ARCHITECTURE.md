...
Automatic delivery uses one extension-wide no-queue critical section from clipboard preparation through the paste result plus one immediate-fail native paste mutex. A second activation is declined before it can replace the clipboard; its trigger plus ordinary Space remain and a concise retry-later delivery-busy message must not claim that second Snippet was copied. The automatic-paste result taxonomy is `paste-issued`, `clipboard-only`, `unsafe-focus`, `not-foreground`, `clipboard-changed`, `unsafe-keyboard-state`, `busy`, `native-unavailable`, `input-injection-failed`, and `indeterminate`. Automatic success is announced as `Paste sent`. Once clipboard success is confirmed, every automatic non-success falls back to the existing truthful copied notice, with `(trigger unchanged)` when cleanup was not authorized. The clipboard is not cleared after success, so a later user `Ctrl+V` naturally inserts it again.

The future Settings field is `snippetPasteMode: 'clipboard-only' | 'automatic'`, defaulting to `clipboard-only`, in the existing singleton Settings aggregate/repository. It requires no new store, index, or Dexie version. Backup v5 stays frozen; M14-K.2 must introduce strict Backup v6 for new exports if the preference is persisted in backups, and every v1-v5 import supplies `clipboard-only`. Windows automatic mode depends on the existing optional native companion/permission. Unsupported platforms and unavailable helpers remain fully supported in clipboard-only mode.

M14-K.1 defines this architecture only. M14-K.2 owns Settings/Backup v6, protocol v2, extension/native implementation, automated fault/race coverage, and timing instrumentation. M14-K.3 owns real Crisp/Intercom Text/Image and adversarial focus validation. No current production behavior changes during M14-K.1.
