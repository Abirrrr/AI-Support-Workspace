...
`DATABASE_SCHEMA.md` is authoritative for the implemented physical schema, repository semantics, errors, transactions, testing, and migration policy. The Milestone 3 physical schema includes Knowledge Entry and Snippet Entry records. M11 now approves the typed Settings aggregate and the future implementation's version 2 singleton store; the physical database remains version 1 until M11 implementation.
...
The platform architecture remains approved and frozen: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. Milestones 1 through 10 are implemented and validated. The M10 implementation checkpoint is `6093361` (`feat: add selected-text capture shortcut`), and repository continuity is synchronized through `37ad72f` (`docs: synchronize project state after M10`). Final M10 validation passed the focused M10/Workspace tests, the full 171-test Vitest suite with one opt-in live Ollama test skipped, static and build gates, generated-output validation, and the complete real Chrome workflow. The documented repeated-panel keyboard-routing limitation remains non-blocking. Milestone 11 — Settings is current and architecture-defined but not implemented. Its scope is exactly one optional saved default Ollama model, the options-page Settings form, typed load/save boundaries, new-session Workspace initialization, and the approved Dexie version 2 singleton migration.
