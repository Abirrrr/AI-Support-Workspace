...
The platform architecture remains approved: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, Tiptap v3 as constrained UI infrastructure, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. M14-N.3 makes Manual Backup v7 Export plus local `lastSuccessfulBackupAt` and a 30-day advisory reminder the current backup architecture. Dexie remains v6, Backup remains v7, historical automatic state remains dormant, and production needs no filesystem authority or `alarms` permission. M14-P.1 through M14-P.5 are complete approved checkpoints; M14-P.5 is synchronized at `d94181746cff07632fba1cdb9c6d874b569b4f21`, and M14-P.5.1 records the documentation-only continuity transition. M14-O is implemented with automated validation passing and awaits Principal review. It adds no production generator, generated-tag UI, permission, dependency, schema/Backup version, startup/background work, or delivery-path work. The final M14-P gate with Text performance regression verification follows an approved M14-O checkpoint, then M15. The canonical `;trigger + Space`, delivery, image quality, persistence versions, and M15 AI-function ownership remain unchanged.
