...
Decisions 40 and 54 approve a future shell correction without changing the current implementation. The extension toolbar action will open/toggle the existing global AI Support Workspace Side Panel directly, with no intermediate popup. Chrome's supported mechanism is idempotent extension-startup `chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true })`. Chrome owns the native toolbar interaction; the implementation does not create custom `chrome.action.onClicked` toggle state. The existing `sidePanel` permission is sufficient for that behavior; no new permission is expected solely for this correction.

The Side Panel remains the focused daily Workspace rather than becoming the full management application. Its header places a compact icon-only Settings button beside the **AI Support Workspace** title. The button invokes `chrome.runtime.openOptionsPage()` and has the accessible name `Open Settings and Libraries`, native keyboard activation, visible focus, a sufficient hit target, and safe non-blocking failure announcement. It is not a full-width **Open Libraries** action and has no dropdown/menu. Knowledge compatibility, Text/Image Snippets, Settings, Import / Export, automatic-backup controls, paste behavior, model/provider controls, and future management remain owned by the options page.

The future implementation must inspect WXT's generated manifest and action configuration, remove or retire `action.default_popup` and the popup entry point cleanly, and retain or explicitly declare a popup-free `action` so the toolbar icon remains. It must preserve `side_panel.default_path`, global panel scope, keyboard shortcut behavior, `options_ui`, options-page boundaries, and least-privilege permissions. Current build tests that require `popupPage`, popup navigation tests, and popup-side-panel tests must be retired or replaced with manifest behavior, Settings-control, Options-navigation, and real-Chrome action-click coverage. Decision 54 assigns this work to the future M15 AI Workspace implementation scope.
...
The implemented Settings field is `snippetPasteMode: 'clipboard-only' | 'automatic'`, defaulting to `clipboard-only`, in the existing singleton Settings aggregate/repository. It adds no store, index, or Dexie version. Backup v5 stays frozen; strict Backup v6 is the current export format and every v1-v5 import supplies `clipboard-only`. Windows automatic mode depends on the existing optional native companion/permission. Unsupported platforms and unavailable helpers remain fully supported through clipboard preparation and manual `Ctrl+V` fallback.
...
The Image hot path remains authoritative load/ownership validation → metadata-before-decode safety → PNG direct return or JPEG/WebP browser decode and PNG re-encode → base64 Native Messaging serialization → one-shot native parse/WIC decode → registered PNG plus CF_DIBV5 clipboard ownership → the shared optional paste boundary. Direct PNG preparation is efficient. Non-PNG conversion and one-shot host process churn are measured class-C architectural opportunities, not defects and not justification to change Decision 43's request-scoped model during closeout. Decision 42 limits remain unchanged. Production installer/registration, signing, updater, and version migration remain a separate distribution architecture boundary.

### Approved Future AI Drafting and Snippet-Hardening Architecture

Decisions 46–48 define future product architecture; they do not describe the current runtime. Today, M6 retrieval still reads both implemented repositories, M7/M9 Prompt Builder still accepts Knowledge and Snippet collections, the options page still renders Knowledge CRUD, M9/M11 still use a free-text model value, Merchant Context is text-only, and Dexie physical version 5 plus Backup v6 remain current.
...
The compact future Side Panel keeps inputs before output, uses auto-growing bounded Context and Guidance / Gist fields, places a provider-independent Model dropdown beside Generate, preserves editable output and Copy, and removes permanent explanatory copy from the primary workflow. Request-scoped removable Context Images are a separate transient M15 input domain, never an Image Snippet or Knowledge record. Provider capability/serialization remains outside React and outside this documentation task.

Snippet hardening is assigned to M14-L through M14-P under Principal-approved Decisions 50–53, with M14-L.1 reconciling feasibility order and Decision 54 preserved separately. M14-L/L.1 is documentation-only: current runtime remains Dexie v5, Backup v6, authored `SnippetEntry.tags`, no usage store, no directory handle, no alarm, and no automatic tag call.

#### M14-L Coordinated Hardening Data Architecture

M14-M.0 is a mandatory real-Chrome selected-folder feasibility gate before M14-M.1. It uses only a task-scoped scratch IndexedDB boundary and an exact self-owned test file; it adds no production migration or scheduler permission. It must prove Options-page `showDirectoryPicker()` under explicit user activation, handle structured-clone persistence, relevant extension/service-worker lifecycle recovery, truthful `queryPermission({mode:'readwrite'})`, already-granted background-context reuse without prompting, user-gesture-only recovery from `prompt`/`denied`, exact create/close/reopen/read/identity/remove behavior, unrelated-file preservation, and safe revocation/unavailability. Gate failure stops M14-M.1 and returns to Principal architecture review.

Only after that PASS will M14-M.1 own one coordinated physical Dexie v6 / Backup v7 evolution. Separate `snippetUsageStats` and `snippetGeneratedMetadata` stores keep frequent operational writes and fallible AI metadata out of authored `SnippetEntry`, catalog publication, and authored timestamps. The intended singleton `automaticBackupState` stores only local environmental state—selected `FileSystemDirectoryHandle`, destination/backup-set identity, next due/attempt/success/failure information, run lease, and verified managed-file manifest—but its final physical declaration is committed only from the feasibility-proven model. Settings adds the portable preferred `automaticBackupCadence`, while nonportable handle/permission/operational-enabled/schedule/manifest state is excluded from backup serialization.

Strict Backup v7 extends rather than rewrites v6. It retains Knowledge compatibility, Text/Image Snippets, assets, authored tags, model and paste settings, and adds generated metadata, usage statistics, preferred cadence, plus required backup/creation ownership metadata while retaining the 96 MiB canonical byte limit. V1–v6 map absent new values safely. Restore validates every reference and source fingerprint and atomically replaces the portable data stores, then clears nonportable automatic-backup authority. Operational automatic writing is derived, never restored: it requires a non-`off` cadence, a local handle, and current granted read/write permission. Thus restored Daily/Weekly plus no usable local authorization stays inactive, never prompts or rapidly loops in background, reports `Backup location needs attention`, and preserves manual Export until an explicit user gesture authorizes a folder; restored Off remains intentionally inactive.
...
The platform architecture remains approved: WXT, Manifest V3, TypeScript, React, Tailwind CSS, pnpm, Dexie, Tiptap v3 as constrained UI infrastructure, React Context and Hooks, Vitest, Playwright, ESLint, Prettier, Husky, and lint-staged. M14-J's generic ordinary and Shadow-DOM editor resolution, safe serialization, destination evidence, and lifecycle recovery remain complete. M14-K.3 Principal-approved Decision 45's optional additive Windows automatic paste, strict Backup v6, and protocol v2 while preserving persisted `SnippetContent`, deterministic clipboard representations, Decisions 42–44, protocol-v1 Image clipboard behavior, and Dexie v5. Manual native `Ctrl+V` remains a permanent supported mode/fallback. M14-K is complete and closed at implementation checkpoint `e34cd76`; Decisions 46–49 and the unassigned post-M14 Snippet-hardening gate remain future work.
